...
public:
    ...
    /// NUM IN-LINE /// CALCULATOR MODE: STANDARD BANK ROUND OF THE OBJECT DECIMAL DIGITS TO THE SPECIFIED NUMBER (LOSS PRECISION), CODE: NUM a("000123.12345678900"); a.round_bank(4).print("\n"); //123.1234
    NUM& round_bank(i32 d = 2) {
        ...
        as = exp2num(result);
        asp = split(as, ".");
        if (d > 0 && d >= strlen(asp[1])) { free(asp[0]); free(as); return result = *this; } //CHECKING DECIMAL PART AND RIGHT LIMIT
