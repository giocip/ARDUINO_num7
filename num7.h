/** DEVELOPED ON MCU SAMD21G18A (256KB Flash, 32KB RAM) AND ARDUINO IDE Version: 2.3.2 CLI Version: 0.35.3 */

/**
 * @file   num7.h
 * @author Giovanni Cipriani <giocip7@gmail.com>
 * @date   2025
 * @brief  num7 C++ 32-BIT LIBRARY, ARBITRARY-PRECISION GENERAL PURPOSE ARITHMETIC-LOGIC DECIMAL CLASS FOR 32-BIT ARDUINO ARCHITECTURE
 *
 * @see https://github.com/giocip/ARDUINO_num7
 */
#include <Arduino.h>

#if defined(ARDUINO_ARCH_ESP32)
#else
  extern "C" char* sbrk(int incr);    //MEMORY FREE ARM
#endif

extern int __heap_start,*__brkval; //MEMORY FREE AVR
#ifndef num7_h
#define num7_h
#define RAM_SYS 1024 //(BYTE) FREE RAM MEMORY FOR ARDUINO SYSTEM BOARD 
...
char* bits(const char*, int bin = true);
NUM& divf(NUM*, NUM*, i32 dp = 30);
NUM& divf_signed(NUM*, NUM*, int dp = 32);
char* divfs(const char*, const char*, i32 d = 30, int check = 1);
Division* divis(const char*, const char*, int check = 1);
char* exp2num(NUM&);
char* exp2str(const char*);
char* expEQ(const char*, const char*);
char* format(NUM, int d = 2, char SEP1000 = ',', int SIGN = 0);
char* format0(NUM&, int d = 2);
char* format_int(NUM&);
int eq_abs(NUM*, NUM*);
int gt_abs(NUM*, NUM*);
int ne(NUM*, NUM*);
...
NUM& POW(NUM&, i32, i32);
NUM& POW(NUM&, NUM&, i32);
NUM& POW_MOD(NUM&, i32, i32);
NUM& pwrf_signed(NUM *, i32);
char* pwri(I32, I32);
char* rstripf0(char*);
...
char* stripf0(char*);
char* stripi0(char*);
char* strpads(const char*, i32);
char* strpads0(i32 qty);
char* subis(const char*, const char*, int check = 1);
NUM&  subf_signed(NUM*, NUM*);
NUM&  subf(NUM*, NUM*);
char* subfs(const char*, const char*, int check = 1);
char* trunks(const char*, i32);
int error();
int error_set();
int error_clear();
int raise(const char*, const char*, int video);
NUM& add(NUM&, NUM&);
...
NUM& mul(int, int);
NUM& div(NUM&, NUM&, int pd = 40);                //DEFAULT pd = 40
NUM& div(const char*, const char*, int pd = 40); //DEFAULT pd = 40
NUM& div(String, String, int pd = 40);          //DEFAULT pd = 40
NUM& div(int, int, int pd = 40);               //DEFAULT pd = 40
NUM& pow(NUM&, NUM&);
...
NUM& xy(int, int);
NUM& sqr(NUM&, int b = 9);
NUM& sqr(const char*, int b = 9);
NUM& sqr(String, int b = 9);
NUM& sqr(int, int b = 9);
NUM& root_i(NUM&, int b = 3);
NUM& root_i(const char*, int b = 3);
NUM& root_i(String, int b = 3);
NUM& root_i(int, int b = 3);
NUM& x2(NUM&);
NUM& x2(const char*);
NUM& x2(String);
NUM& x2(int);
...
NUM& pi();
NUM& e();
NUM& spinoff(NUM&, NUM);                              //DEFAULT all = 1
NUM& spinoff(const char*, const char* all = "1.0");  //DEFAULT all = "1.0"
NUM& spinoff(String, String all = "1.0");           //DEFAULT all = "1.0"
NUM& spinoff(int, int all = 1);                    //DEFAULT all = 1
NUM& spinon(NUM&, NUM);                              //DEFAULT a = 1
NUM& spinon(const char*, const char* all = "1.0");  //DEFAULT a = "1.0"
NUM& spinon(String, String all = "1.0");           //DEFAULT a = "1.0"
NUM& spinon(int, int all = 1);                    //DEFAULT a = 1
I32 freemem(); 
NUM& perf(NUM, NUM, int time = 1);
NUM& fact(int);
int fact_digits(int);
char *FACT(int); 
...
int print(i64 a, const char* postfix = "");
NUM& round(NUM&, i32 d = 2);         //DEFAULT d = 2
NUM& round(const char*, i32 d = 2); //DEFAULT d = 2
NUM& round(String, i32 d = 2);     //DEFAULT d = 2
NUM& round(int, i32 d = -1);      //DEFAULT d = -1
NUM& round_bank(NUM&, i32 d = 2);         //DEFAULT d = 2
NUM& round_bank(const char*, i32 d = 2); //DEFAULT d = 2
NUM& round_bank(String, i32 d = 2);     //DEFAULT d = 2
NUM& round_bank(int, i32 d = -1);      //DEFAULT d = -1
NUM& round_floor(NUM&, i32 decs = 0);         //DEFAULT decs = 0
NUM& round_floor(const char*, i32 decs = 0); //DEFAULT decs = 0
NUM& round_floor(String, i32 decs = 0);     //DEFAULT decs = 0
NUM& round_floor(int, i32 decs = 0);       //DEFAULT decs = 0
NUM& round_ceil(NUM&, i32 decs = 0);         //DEFAULT decs = 0
NUM& round_ceil(const char*, i32 decs = 0); //DEFAULT decs = 0
NUM& round_ceil(String, i32 decs = 0);     //DEFAULT decs = 0
NUM& round_ceil(int, i32 decs = 0);       //DEFAULT decs = 0
NUM& inc(NUM&, NUM&);
...
public:
    ...
    i32   E;      /**< SIGNED INTEGER NUMBER EXPONENT PART */
    int   S;     /**< SIGN=0 FOR POSITIVE OR SIGN=1 FOR NEGATIVE */
    char* CE;   /**< C._eE EXPONENT */
    i32 len_I; /**< NUMBER COEFFICIENT STRING INTEGER PART LENGTH */
    i32 len_F;/**< NUMBER COEFFICIENT STRING FLOATING-POINT PART LENGTH */
    ...
    /////// 1 ///////
    NUM(int n = 0) { //CONSTRUCTOR METHOD DEFAULT TO ZERO, int OVERLOADED
        if (!n) {   //RAM DYNAMIC ALLOCATION
            char* str = (char*)malloc(8 * sizeof(char)); //NULL, DOT, 'e' 
            if (freemem() < RAM_SYS) reset();
            strcpy(str, "0.0e0"); this->CE = str; //ZERO EQUALIZATION => 0.0e0
            this->E = 0; this->S = 0; this->len_I = 1; this->len_F = 1;
            this->C = (char*)malloc(4 * sizeof(char)); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
            strcpy(this->C, "0.0");
            return;
        }
        char** p;
        ...
        this->C = (char*)malloc(((i32)strlen(p[0]) + 32) * sizeof(char)); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
        strcpy(this->C, p[0]);
        this->E = (i32)atoll(p[1]); free(p[0]);
        p = split(this->C, "."); this->len_I = (i32)strlen(p[0]); this->len_F = (i32)strlen(p[1]);
        free(p[0]); 
        return;
    }
    /////// 2 ///////
    NUM(String s) { //CONSTRUCTOR METHOD, String OVERLOADED
        static NUM num;
        num = s.c_str();
        this->C = num.C; num.C = NULL; this->E = num.E; this->S = num.S; this->CE = num.CE; num.CE = NULL;
        this->len_I = num.len_I; this->len_F = num.len_F;
    ...
    /////// 3 ///////
    NUM(const char* s) { //CONSTRUCTOR METHOD
        //Serial.println("CONSTRUCTOR IN ACTION...");
        ...
        char* str = (char*)malloc(((i32)strlen(s) + 32) * sizeof(char)); //NULL, SIGN, DOT, 'e', 'E', 20 DIGIT EXPONENT 
        if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
        strcpy(str, s);
        ...
        if (is_strfmt_float(str)) {
            stripf0(str);
            if (this->S && !strcmp(str, "0.0")) { free(str); raise("ARGUMENT VALUE, ZERO CAN NOT BE SIGNED => NUM CONSTRUCTOR", s); *this = 0; return; } //CHECK -0.07
            this->CE = str;
            char* t = num2exp(str);
            strcpy(str, t); free(t);
            p = split(str, "e");
            ...
            this->E = (i32)atoll(p[1]); free(p[0]);
            p = split(this->C, "."); this->len_I = (i32)strlen(p[0]); this->len_F = (i32)strlen(p[1]); free(p[0]);
            return;
        }
            ...
            strcpy(this->CE, this->C); strcat(this->CE, "e"); strcat(this->CE, i32str(E));
            p = split(this->C, ".");
            this->len_I = (i32)strlen(p[0]);
            this->len_F = (i32)strlen(p[1]);
            free(p[0]);
            return;
        }
    ...
    NUM(const NUM& arg) { //COPY CONSTRUCTOR METHOD USED WHEN A NUM VAR IS BOTH DECLARED AND BY A VARIABLE ASSIGNED. (ALSO BY A FUNCTION OR WHEN PASSING A FUNCTION ARGUMENT)
        //Serial.println("COPY CONSTRUCTOR METHOD: "); arg.print();
        C  = (char*)malloc(((i32)strlen(arg.C)  + 1) * sizeof(char)); if (freemem() < RAM_SYS) reset();  //RAM DYNAMIC ALLOCATION
        CE = (char*)malloc(((i32)strlen(arg.CE) + 1) * sizeof(char)); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    ...
    ~NUM() { 
        ...
        return;
        //Serial.println("DESTRUCTION ENDED!");
    }  //DESTRUCTOR METHOD
    ...
    /// NUM IN-LINE /// ASSIGNMENT ALLOWS ALLOW RIGHT OPERAND TO BE NUM, CODE: NUM a, b("3.14"); a = b; print("a = ", a, "  b = "); print(b, "\n"); //a = 3.14  b = 3.14
    NUM& operator=(NUM& b) { 
        if (this == &b) return *this;
        free(C); free(CE); //FREE OLD RAM MEMORY
        //ALLOCATE NEW RAM MEMORY
        this->C = (char*)malloc(((i32)strlen(b.C) + 1) * sizeof(char));   if (freemem() < RAM_SYS) reset();  //RAM DYNAMIC ALLOCATION
        this->CE = (char*)malloc(((i32)strlen(b.CE) + 1) * sizeof(char)); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    ...
    /// NUM IN-LINE /// ASSIGNMENT ALLOWS RIGHT OPERAND TO BE *char, CODE: NUM a; a = "-1234567890.0"; print("a = ", a, "\n"); //a = -1234567890.0
    ...
    /// NUM IN-LINE /// NOT EQUALITY, CODE: NUM a(2); int b(3); print(a != b, "\n"); //1 (true)
    int operator!=(int b) {
        NUM B(b);
        return ne(this, &B);
    }
    ...
    /// NUM IN-LINE /// ADDITION, CODE: NUM a("3.14"); int b(3); NUM sum = a + b; print(sum, "\n"); //6.14
    NUM& operator+(int b) { 
        static NUM SUM;
        NUM B(b);
        if (!strcmp(this->C, "0.0")) return SUM = B; 
    ...
    NUM& operator^(NUM &e) {
        ...
        return pwrf_signed(this, e.to_i32());
    }
    ...
    /// NUM IN-LINE /// POWER AND ASSIGNMENT, CODE: NUM a("25.00"); a ^= "-8.0"; print(a, "\n"); //0.0000000000065536
    NUM& operator^=(const char* e) {
        return *this = *this ^ e;
    }
    ...
    /// NUM IN-LINE /// NEED: free(p[0]) (C_I => p[0]) RETURN COEFFICIENT INTEGER PART OF NUM, CODE: NUM a(2 + 3); char** p = a.I_(); a.print_fields(); print(p[0], "\n"); free(p[0]); //5
    char** I_() { 
        return split(C, ".");
    }
    ...
    char* get_NUM() {
        char* s = (char*)malloc(((i32)strlen(CE) + 2) * sizeof(char)); //SIGN + NULL
        if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    ...
    char* bits(int bin = true) { //DEFAULT ARGs: (int bin = true)
    ...
    char* format(int d = 2, char SEP1000 = ',', int SIGN = 0) {
    ...
    char* to_bin(int bin = true) { //DEFAULT ARGs: (int bin = true)
    ...
    /// NUM IN-LINE /// CALCULATOR MODE: RETURN SCIENTIFIC NOTATION NUMBER OBJECT, CODE: NUM a("123.0e45"), b("678.0e9"), c = a * b; NUM d = c.into_sci(); d.print_fields(); //S=0 CE=8.3394e58 C=8.3394 E=58 len_I=1 len_F=4 SCIENTIFIC NOTATION
    NUM& into_sci() {
        static NUM N;
        char* n = exp2num(*this);
        N = n; free(n);
    ...
    /// NUM IN-LINE /// CALCULATOR MODE: NORMALIZE OBJECT ITSELF INTO SCIENTIFIC NOTATION NUMBER, CODE: NUM a("123.0e45"), b("678.0e9"), c = a * b; c.into_exp(); c.print_fields(); //S=0 CE=8.3394e58 C=8.3394 E=58 len_I=1 len_F=4 SCIENTIFIC NOTATION
    NUM& into_exp() {
        char* n = exp2num(*this);
        *this = n; free(n);
        return *this;
    ...
    /// NUM IN-LINE /// CONVERT NUM TO I64 (UNSIGNED 64-BIT INTEGER) RANGE BETWEEN 0 TO 18446744073709551615, CODE: NUM a("18446744073709551614.0"); I64 A = a.to_I64(); print(A+1, "\n"); //18446744073709551615 
    I64 to_I64() {
        static char* p, * ram;
        static I64 L, result;
    ...
    char* to_exp() {
        char* ram = (char*)malloc((len_I + len_F + 8) * sizeof(char)); //'.', 'e', '+', '-', 'NULL'
        if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    ...
    /// NUM IN-LINE => CONVERT NUM TO double TYPE (LOSS PRECISION) CODE: Serial.println( add("0.000000001", "0.000000006").to_double(), 9 ); //0.000000007
    double to_double() {
        static char* ram;
        static double d;
        char * end;
    ...
    int print(const char *postfix = "", int SIGN = 0) {
        static int n;
        char* t = exp2num(*this);
        char* s = (char*)malloc((strlen(t) + 1) * sizeof(char)); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
        strcpy(s, t); free(t);
        if (SIGN) {
            if (this->S && s[0] != '-') { n = Serial.print("-" + String(s) + String(postfix)); free(s); return n; }
            else if (this->S)           { n = Serial.print(      String(s) + String(postfix)); free(s); return n; }
            else if (strcmp(s, "0.0"))  { n = Serial.print("+" + String(s) + String(postfix)); free(s); return n; }
            n = Serial.print(String(s) + String(postfix));
        }
        else n = Serial.print(String(s) + String(postfix));
        free(s);
        return n;
    }
    /// NUM IN-LINE /// PRINT NUM EXPONENTIAL FORMAT (SIGNED), CODE: NUM a("-123.0e-3"); a.print_exp("\n"); //-123.0e-3
    int print_exp(const char* postfix = "") {
        return Serial.print(String(S ? "-" : "") + CE + String(postfix));
    }
    /// NUM IN-LINE /// PRINT NUM FIELDS, CODE: NUM a("-123.0e-3"); a.print_fields(); //S=1 CE=123.0e-3 C=123.0 E=-3 len_I=3 len_F=1
    int print_fields() {
        String tot = String("S=") + i32str(S) + String(" CE=") + CE + String(" C=") + C;
        tot += String(" E=") + i32str(E);
        tot += String(" len_I=") + i32str(len_I);
        tot += String(" len_F=") + i32str(len_F);
        return Serial.println(tot.c_str());
    }
    ...
    /// NUM IN-LINE /// RETURN TRUNCATION OF THE OBJECT TO THE SPECIFIED NUMBER DECIMALS (LOSS PRECISION), CODE: NUM a("123.456"); a = a.trunk(2); print(a, "\n"); //123.45
    NUM& trunk(i32 decs = 19) { //DEFAULT ARGs: (decs = 19)
        static NUM T;
        int SIGN = this->S;
        char* t0 = exp2str(CE);
        char* t = trunks(t0, decs); free(t0);
        char* ce = (char*)malloc(((i32)strlen(t) + (i32)strlen(i32str(this->E)) + 16) * sizeof(char)); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
        t0 = num2exp(t);
    ...
    /// NUM IN-LINE /// CALCULATOR MODE: ROUND FLOOR OF THE OBJECT DECIMAL DIGITS TO THE SPECIFIED NUMBER (LOSS PRECISION), CODE: NUM a("123.456"); a = a.round_floor(0); print(a, "\n"); //123.0
    NUM& round_floor(i32 decs = 0) {
      return trunk(decs);
    }
    /// NUM IN-LINE /// CALCULATOR MODE: RETURN ROUND CEIL OF THE OBJECT TO THE SPECIFIED DIGIT NUMBER (LOSS PRECISION), CODE: NUM a("123.456"); a = a.round_ceil(0); print(a, "\n"); //123.0
    NUM& round_ceil(i32 decs = 0) {
        static NUM t, t2;
        NUM n_10("10.0");
    ...
    /// NUM IN-LINE /// CALCULATOR MODE: RETURN STANDARD ROUND OF THE OBJECT DECIMAL DIGITS TO THE SPECIFIED NUMBER (LOSS PRECISION), CODE: NUM a("000123.12345678900"); a.round(5).print("\n"); //123.12346
    NUM& round(i32 d = 2) {
        /**
    ...
    /// NUM IN-LINE /// CALCULATOR MODE: STANDARD BANK ROUND OF THE OBJECT DECIMAL DIGITS TO THE SPECIFIED NUMBER (LOSS PRECISION), CODE: NUM a("000123.1234500"); a.round_bank(4).print("\n"); //123.1234
    NUM& round_bank(i32 d = 2) {
        static char* as;
        static char** asp;
        static NUM result;
        ...
        as = exp2num(result);
        asp = split(as, ".");
        if (d > 0 && d >= (i32)strlen(asp[1])) { free(asp[0]); free(as); return result = *this; } //CHECKING DECIMAL PART AND RIGHT LIMIT
    ...
    /// NUM IN-LINE /// CALCULATOR MODE: WITH THE ALL, RETURN THE PERCENTAGE OF SPECIFIED RATE, CODE: NUM all("1_000.0"), rate(2); all.pct(rate).round().print("\n"); //20.0
    NUM& pct(NUM &rate) {
        return this->xe10(-2) * rate;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: WITH THE ALL, RETURN THE PERCENTAGE OF SPECIFIED RATE, CODE: NUM all("1_000.0"); all.pct("2.0").round().print("\n"); //20.0
    NUM& pct(const char* rate) {
        return this->xe10(-2) * rate;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: WITH THE ALL, RETURN THE PERCENTAGE OF SPECIFIED RATE, CODE: NUM all("1_000.0"); all.pct(String("2.0")).round().print("\n"); //20.0
    NUM& pct(String rate) {
        return this->xe10(-2) * rate;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: WITH THE ALL, RETURN THE PERCENTAGE OF SPECIFIED RATE, CODE: NUM all("1_000.0"); all.pct(2).round().print("\n"); //20.0
    NUM& pct(int rate) {
        return this->xe10(-2) * rate;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: WITH THE ALL, RETURN THE RATE OF SPECIFIED PERCENTAGE, CODE: NUM ALL("1_000.0"), pct(20); ALL.rate(pct).print("\n"); //2.0
    NUM& rate(NUM &pct) {
        return 100 / *this * pct;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: WITH THE ALL, RETURN THE RATE OF SPECIFIED PERCENTAGE, CODE: NUM ALL("1_000.0"); ALL.rate("20.0").print("\n"); //2.0
    NUM& rate(const char* pct) {
        return 100 / *this * pct;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: WITH THE ALL, RETURN THE RATE OF SPECIFIED PERCENTAGE, CODE: NUM ALL("1_000.0"); ALL.rate(String("20.0")).print("\n"); //2.0
    NUM& rate(String pct) {
       return 100 / *this * pct;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: WITH THE ALL, RETURN THE RATE OF SPECIFIED PERCENTAGE, CODE: NUM ALL("1_000.0"); ALL.rate(20).print("\n"); //2.0
    NUM& rate(int pct) {
        return pct * 100 / *this;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: WITH THE PERCENTAGE RETURN THE ALL OF SPECIFIED RATE, CODE: NUM pct(20), rate(2); pct.all(rate).print("\n"); //1000.0
    NUM& all(NUM &rate) {
        return this->xe10(2) / rate;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: WITH THE PERCENTAGE RETURN THE ALL OF SPECIFIED RATE, CODE: NUM pct(20); pct.all("2.0").print("\n"); //1000.0
    NUM& all(const char* rate) {
        return this->xe10(2) / rate;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: WITH THE PERCENTAGE RETURN THE ALL OF SPECIFIED RATE, CODE: NUM pct(20); pct.all(String("2.0")).print("\n"); //1000.0
    NUM& all(String rate) {
        return this->xe10(2) / rate;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: WITH THE PERCENTAGE RETURN THE ALL OF SPECIFIED RATE, CODE: NUM pct(20); pct.all(2).print("\n"); //1000.0
    NUM& all(int rate) {
        return this->xe10(2) / rate;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: RETURN THE PERTHOUSAND TO SPECIFIED RATE, CODE: NUM all("10_000.0"), rate(2); all.pth(rate).print("\n"); //20.0
    NUM& pth(NUM & rate) {
        return this->xe10(-3) * rate;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: RETURN THE PERTHOUSAND TO SPECIFIED RATE, CODE: NUM all("10_000.0"); all.pth("2.0").print("\n"); //20.0
    NUM& pth(const char* rate) {
        return this->xe10(-3) * rate;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: RETURN THE PERTHOUSAND TO SPECIFIED RATE, CODE: NUM all("10_000.0"); all.pth(String("2.0")).print("\n"); //20.0
    NUM& pth(String rate) {
        return this->xe10(-3) * rate;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: RETURN THE PERTHOUSAND TO SPECIFIED RATE, CODE: NUM all("10_000.0"); all.pth(2).print("\n"); //20.0
    NUM& pth(int rate) {
        return this->xe10(-3) * rate;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: WITH THE ALL, RETURN THE RATE TO SPECIFIED PERTHOUSAND, CODE: NUM all_th(10000), pth(20); all_th.rate_th(pth).print("\n"); //2.0
    NUM& rate_th(NUM& pth) {
        return pth.xe10(3) / *this;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: WITH THE ALL, RETURN THE RATE TO SPECIFIED PERTHOUSAND, CODE: NUM all_th(10000); all_th.rate_th("20.0").print("\n"); //2.0
    NUM& rate_th(const char* pth) {
        return (1 / *this).xe10(3) * pth;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: WITH THE ALL, RETURN THE RATE TO SPECIFIED PERTHOUSAND, CODE: NUM all_th(10000); all_th.rate_th(String("20.0")).print("\n"); //2.0
    NUM& rate_th(String pth) {
        return (1 / *this).xe10(3) * pth;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: WITH THE ALL, RETURN THE RATE TO SPECIFIED PERTHOUSAND, CODE: NUM all_th(10000); all_th.rate_th(20).print("\n"); //2.0
    NUM& rate_th(int pth) {
        return (1 / *this).xe10(3) * pth;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: WITH THE PERTHOUSAND RETURN THE ALL TO SPECIFIED RATE, CODE: NUM pth(20), rate_th(2); pth.all_th(rate_th).print("\n"); //10000.0
    NUM& all_th(NUM& rate_th) {
        return this->xe10(3) / rate_th;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: WITH THE PERTHOUSAND RETURN THE ALL TO SPECIFIED RATE, CODE: NUM pth(20); pth.all_th("2.0").print("\n"); //10000.0
    NUM& all_th(const char * rate_th) {
        return this->xe10(3) / rate_th;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: WITH THE PERTHOUSAND RETURN THE ALL TO SPECIFIED RATE, CODE: NUM pth(20); pth.all_th(String("2.0")).print("\n"); //10000.0
    NUM& all_th(String rate_th) {
        return this->xe10(3) / rate_th;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: WITH THE PERTHOUSAND RETURN THE ALL TO SPECIFIED RATE, CODE: NUM pth(20); pth.all_th(2).print("\n"); //10000.0
    NUM& all_th(int rate_th) {
        return this->xe10(3) / rate_th;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: RETURN THE SPIN-ON OF SPECIFIED RATE, CODE: NUM factory_price("44.74"); NUM rate("22.00"); factory_price.spinon(rate).round().print("\n"); //54.58
    NUM& spinon(NUM& rate) {
        return *this * (rate + 100).shift(-2);
    }
    /// NUM IN-LINE /// CALCULATOR MODE: RETURN THE SPIN-ON OF SPECIFIED RATE, CODE: NUM factory_price("44.74"); factory_price.spinon("22.0").round().print("\n"); //54.58
    NUM& spinon(const char* rate) {
        NUM RATE(rate);
        return *this * (RATE + 100).shift(-2);
    }
    /// NUM IN-LINE /// CALCULATOR MODE: RETURN THE SPIN-ON OF SPECIFIED RATE, CODE: NUM factory_price("44.74"); factory_price.spinon(String("22.0")).round().print("\n"); //54.58
    NUM& spinon(String rate) {
        NUM RATE(rate);
        return *this * (RATE + 100).shift(-2);
    }
    /// NUM IN-LINE /// CALCULATOR MODE: RETURN THE SPIN-ON OF SPECIFIED RATE, CODE: NUM factory_price("44.74"); factory_price.spinon(22).round().print("\n"); //54.58
    NUM& spinon(int rate) {
        NUM RATE(rate);
        return *this * (RATE + 100).shift(-2);
    }
    /// NUM IN-LINE /// CALCULATOR MODE: RETURN THE SPIN-OFF OF SPECIFIED RATE, CODE: NUM asus("842.00"), rate("22.0"); asus.spinoff(rate).round().print("\n"); //690.16
    NUM& spinoff(NUM &rate) { 
        return *this / (rate + 100).shift(-2);
    }
    /// NUM IN-LINE /// CALCULATOR MODE: RETURN THE SPIN-OFF OF SPECIFIED RATE, CODE: NUM asus("842.00"); asus.spinoff("22.0").round().print("\n"); //690.16
    NUM& spinoff(const char* RATE) {
        NUM rate(RATE);
        return *this / (rate + 100).shift(-2);
    }
    /// NUM IN-LINE /// CALCULATOR MODE: RETURN THE SPIN-OFF OF SPECIFIED RATE, CODE: NUM asus("842.00"); asus.spinoff(String("22.0")).round().print("\n"); //690.16
    NUM& spinoff(String RATE) {
        NUM rate(RATE);
        return *this / (rate + 100).shift(-2);
    }
    /// NUM IN-LINE /// CALCULATOR MODE: RETURN THE SPIN-OFF OF SPECIFIED RATE, CODE: NUM asus("842.00"); asus.spinoff(22).round().print("\n"); //690.16
    NUM& spinoff(int RATE) {
        NUM rate(RATE);
        return *this / (rate + 100).shift(-2);
    }
    /// NUM IN-LINE /// CALCULATOR MODE: RETURN THE INVERSE, CODE: NUM a("10.0"); a.inv().print("\n"); //0.1
    NUM& inv(int decs = 32) { //DEFAULT ARGs: (int decs = 32) 
        return num7::inv(*this, decs);
    }
    /// NUM IN-LINE /// CALCULATOR MODE: MULTIPLY FOR TEN, CODE: NUM a(3); a._10x().print("\n"); //30.0
    NUM& _10x() {
        return this->xe10(1);
    }
    /// NUM IN-LINE /// CALCULATOR MODE: MULTIPLY FOR HUNDRED, CODE: NUM a(3); a._100x().print("\n"); //300.0
    NUM& _100x() {
        return this->xe10(2);
    }
    /// NUM IN-LINE /// CALCULATOR MODE: MULTIPLY FOR THOUSAND, CODE: NUM a(3); a._1000x().print("\n"); //3000.0
    NUM& _1000x() {
        return this->xe10(3);
    }
    /// NUM IN-LINE /// CALCULATOR MODE: DIVIDE FOR TEN, CODE: NUM a(3); a._10div().print("\n"); //0.3
    NUM& _10div() {
        return this->xe10(-1);
    }
    /// NUM IN-LINE /// CALCULATOR MODE: DIVIDE FOR HUNDRED, CODE: NUM a(3); a._100div().print("\n"); //0.03
    NUM& _100div() {
        return this->xe10(-2);
    }
    /// NUM IN-LINE /// CALCULATOR MODE: DIVIDE FOR THOUSAND, CODE: NUM a(3); a._1000div().print("\n"); //0.003
    NUM& _1000div() {
        return this->xe10(-3);
    }
    /// NUM IN-LINE /// CALCULATOR MODE: RETURN THE SQUARE, CODE: NUM a("10.0"); a.x2().print("\n"); //100.0
    NUM& x2() { 
        return *this * *this;
    }
    /// NUM IN-LINE /// CALCULATOR MODE: RETURN THE CUBE, CODE: NUM a("10.0"); a.x3().print("\n"); //1000.0
    NUM& x3() { 
    ...
    NUM& _10y() {
        return num7::pow(10, this->to_i32());
    }
    /// NUM IN-LINE /// CALCULATOR MODE: TWO POWER => CODE: NUM a(6); a._2y().print("\n"); //64.0
    NUM& _2y() {
        return num7::pow(2, this->to_i32());
    }
    /// NUM IN-LINE /// CALCULATOR MODE: e POWER => CODE: NUM a(6); a._ey().round(32).print("\n"); //403.42879349273512260838718054342167
    NUM& _ey() {
        NUM E(e());
        return pwrf_signed(&E, this->to_i32());
    }
    /// NUM IN-LINE /// CALCULATOR MODE: MULTIPLY OR DIVIDE SELF OBJECT FOR 10 POWER NUM, CODE: NUM a("1000.0"), e("-6.0"); a.shift(e).print("\n"); //0.001	
    NUM& shift(NUM e) { 
        return num7::shift(*this, e.to_i32());
    }
    /// NUM IN-LINE /// CALCULATOR MODE: MULTIPLY OR DIVIDE SELF OBJECT FOR 10 POWER STRING, CODE: NUM a("1000.0"); a.shift("-6.0").print("\n"); //0.001	
    NUM& shift(const char *e) { 
        return num7::shift(*this, e);
    }
    /// NUM IN-LINE /// CALCULATOR MODE: MULTIPLY OR DIVIDE SELF OBJECT FOR 10 POWER String, CODE: NUM a("1000.0"); a.shift(String("-6.0")).print("\n"); //0.001	
    NUM& shift(String e) { 
        return num7::shift(*this, e);
    }
    /// NUM IN-LINE /// CALCULATOR MODE: MULTIPLY OR DIVIDE SELF OBJECT FOR 10 POWER int, CODE: NUM a("1000.0"); a.shift(-6).print("\n"); //0.001	
    NUM& shift(int e) { 
        return num7::shift(*this, e);
    }   
    ...
    /// NUM IN-LINE /// CALCULATOR MODE: RETURN OBJECT MULTIPLIED OR DIVIDED FOR 10 POWER int, CODE: NUM a("0.001"); a.xe10(6).print("\n"); //1000.0	
    NUM& xe10(int e) { 
        return num7::xe10(*this, e);
    } 
    /// NUM IN-LINE /// NUM a("10.0"); a.sqrt().print("\n"); //3.162277
    NUM& sqrt(int d = 6) { 
        static char* s, * op, * aI, * aF, *pads;
        static I32 aI_L, s_len, i, Shift;
        static I32 aF_L; //FLOATING DIGIT QUANTITY
        static i32 ds;
        I32 L, L2; //RECURSIVE VARIABLE CANNOT BE STATIC!
        char** P;
        static NUM root, t_op;
        static NUM r, q;
        if (*this < 0) { raise("ARGUMENT VALUE, NEGATIVE => sqrt", sprint_fields()); return root = 0; }        
        d = (d >= 0 ? d : -d);            //abs(d)
        ...
        if (!d) {                      //INTEGER ROOT ALGORITHM
            ...
            L2 = L = (I32)(strlen(s) - 1) >> 1; //TWO DIVISION TO OBTAIN INTEGER ROOT SIZE
            r = "10.0"; 
            r.E = L2 + 1; //+6% => while (L2) { r *= 10; L2--; }
            const char* rE_L = i32str(r.E);
            r.CE = (char*)realloc(r.CE, (r.len_I + r.len_F + 3 + strlen(rE_L)) * sizeof(char)); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
            strcpy(r.CE, r.C); strcat(r.CE, "e"); strcat(r.CE, rE_L);
            q = divf(this, &r, 0); 
            while (r > q) { r = (r + q) * "0.5"; r = r.trunk(0); q = divf(this, &r, 0); } //NEWTON'S METHOD
            free(s);
            return root = r;
        }
        ...
        aI = P[0]; aF = P[1];
        aI_L = (i32)strlen(aI);
        aF_L = (i32)strlen(aF); //FLOATING DIGIT QUANTITY
        if (strlen(aI) == 1 && !strtoul(aI, NULL, 10)) { //0 < n < 1 NUMBER SCOPE (0.0625 => 0.25)
            Shift = (aF_L + 1) >> 1; //TWO DIVISION => DECIMAL POINT POSITION
            ds = d - Shift;
            op = (char*)malloc(sizeof(char) * (ds<0?-ds:ds) * 2 + aF_L + 4); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
            if (aF_L % 2) { strcpy(op, aF); strcat(op, "0"); pads = strpads("00", ds);strcat(op, pads); free(pads); } //ODD (DISPARI)
            else          { strcpy(op, aF);                  pads = strpads("00", ds);strcat(op, pads); free(pads); }//EVEN (PARI)
            free(P[0]); free(s); t_op = strcat(op, ".0"); t_op = t_op.sqrt(0); s = exp2num(t_op);
            s_len = (i32)strlen(s) - 2; //POSTFIX ".0"
            ...
            L = (I32)(strlen(s) - 1) >> 1; //TWO DIVISION TO OBTAIN INTEGER ROOT SIZE
            op = (char*)malloc(sizeof(char) * d * 2 + aI_L + 4); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
            strcpy(op, aI); pads = strpads("00", d); strcat(op, pads); free(pads);
        }
        else { //5.29
            L = (I32)(aI_L + 1) >> 1; //TWO DIVISION TO OBTAIN INTEGER ROOT SIZE
            op = (char*)malloc(sizeof(char) * d * 2 + aI_L + aF_L + 4); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
            if (aF_L % 2) { //FRACTIONAL PART DIGITS ODD (DISPARI) 112.333 => 10.598 d=3
                strcpy(op, aI); strcat(op, aF); strcat(op, "0"); pads = strpads("00", d);strcat(op, pads); free(pads);
            }
            else { //FRACTIONAL PART DIGITS EVEN (PARI) 112.34 => 10.59 d=2
                strcpy(op, aI); strcat(op, aF);                  pads = strpads("00", d); strcat(op, pads); free(pads);
            }
        }
	...
	*(op + L) = '.';
    ...
    NUM& root_i(int i = 3, int d = 3) {
        ...
        d = d < 0 ? -d : d;          //Abs(d)
        if (!i) return iroot = 1;
        if (!(i % 2) && n.is_negative()) { raise("IMPOSSIBLE OPERATION => root_i", i32str(i)); return iroot = 0; } //(ROOT 6 OF -64) NEGATIVE ROOTING WITH EVEN INDEX 
        if (i < 0) { n = num7::inv(n, d); i = -i; }
        int sign = n < 0 ? 1 : 0; //SAVE THE SIGN
	...
	free(sn);
        ...
        while (z < s) { //NEWTON'S METHOD
            ...
            if (error()) raise("d PARAMETER TOO LOW => root_i", i32str(d));
        }
    ...
    NUM& div(const char* b, int dp = 40) {
        static NUM ZERO;
        NUM B(b);
        if (!strcmp(this->C, "0.0")) return ZERO; //(*this) ZERO DIVIDEND => ZERO QUOTIENT
        if (!strcmp(B.C, "0.0")) { 
    ...
    /// NUM IN-LINE /// RETURN FACTORIAL COMPUTATION, CODE: NUM a("5.0001"); a.fact().print("\n"); //120
    NUM& fact() {
        return num7::fact(this->to_i32());
    }
    /// NUM IN-LINE /// RETURN INCREMENTED OBJECT (DEFAULT BY ONE), CODE: NUM a("1.1"); a.inc(3).print("\n"); //4.1
    NUM& inc(int b = 1) {
        return *this = *this + b;
    }
    ...
    /// NUM IN-LINE /// RETURN DECREMENTED OBJECT (DEFAULT BY ONE), CODE: NUM a("1.1"); a.dec(3).print("\n"); //-1.9
    NUM& dec(int b = 1) {
        return *this = *this - b;
    }
...
int raise(const char* msg, const char* value, int video) { ////DEFAULT: int video = 1
    /** RAISE AN ERROR */
    if(video) {
        String B = "ERROR " + String(msg) + ": [" + (strlen(value) ? value : "\"\" => VOID STRING") + String("]\n");
        Serial.println("\n>>>>>>>>>>>>>>>>>>>>>>>> EXCEPTION: num7.h >>>>>>>>>>>>>>>>>>>>>>>>"); 
        Serial.println(B); 
    }
    Serial.flush();
...
/// NUM OUT-LINE /// CHECK IF A NUMERIC STRING IS INTEGER FORMAT NUMBER (UNSIGNED), CODE: print(is_strfmt_int("3"), "\n"); //3
const char* is_strfmt_int(const char* s) {
    /**
    *   \brief  CHECK IF A NUMERIC STRING IS INTEGER FORMAT NUMBER (UNSIGNED)
    *   \param  s STRING
    ...
    *   EXAMPLES:
    *
    *       print(is_strfmt_int("3"), "\n");                   //3
    *       print(is_strfmt_int("00300"), "\n");              //00300
    *       if(!is_strfmt_int("3.0"))  print("null", "\n");  //null
    *       if(!is_strfmt_int("3.14")) print("null", "\n"); //null
    *
    *   \endcode
    */
    I32 i = 0, s_len = strlen(s);
    if (!s_len) return s = NULL; 
    while (i < s_len) { if (!isdigit(s[i])) return s = NULL; i++; }
    return s;
}
/// NUM OUT-LINE /// CHECK IF A NUMERIC STRING IS FLOATING-POINT FORMAT (UNSIGNED), CODE: const char* ff = is_strfmt_float("3.14"); print("floatfmt=", ff);   //floatfmt=3.14
...
/// NUM OUT-LINE /// ABSOLUTE ARBITRARY-PRECISION INTEGER NUMERIC STRING ADDITION (UNSIGNED) NEED: free(), CODE: char *R = addis("3141592654", "2718281828"); print(R, "\n"); free(R); //5859874482
char* addis(const char* op1, const char* op2, int check) { //DEFAULT ARGs: (int check = 1) (check=0 => +30.00%)
    /**
    ...
    *   RUN-TIME ERROR CHECK: YES
    *   NEED: free()
    *   CALL: stripi0 is_strfmt_int raise freemem reset 
    *   TIME: [59us-...]
    *   LIMIT: RAM MEMORY
    *
    *   EXAMPLES:
    ...
    */
    ...
    DIM = (a_L > b_L ? a_L : b_L) * sizeof(char) + 4; //1 => NULL, 1 => CARRY
    a = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();    //RAM DYNAMIC ALLOCATION
    b = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();   //RAM DYNAMIC ALLOCATION
    z = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();  //RAM DYNAMIC ALLOCATION
    r = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    z[0] = '\0';  //"" ZERO STRING LENGTH INITIALIZATION
    r[0] = '\0'; //"" ZERO STRING LENGTH INITIALIZATION
    strcpy(a, op1); if (a[0] == '0') { stripi0(a); a_L = (i32)strlen(a); } //0003 = > 3 (CLEAR ZEROs)
    strcpy(b, op2); if (b[0] == '0') { stripi0(b); b_L = (i32)strlen(b); }
    if (a[0] == '0') { free(a); free(r); free(z); return b; }  //op1 => ZERO ADDEND 1
    if (b[0] == '0') { free(b); free(r); free(z); return a; } //op2 => ZERO ADDEND 2
    if (a_L < b_L) { TEMP = a; a = b; b = TEMP; offset = a_L ; a_L = b_L; b_L = offset; } //IF NEEDED OPERANDS a AND b SWAPPED 
    ...
    for (a_L--; a_L >= 0; a_L--) { //SIMPLE SCHOOL SUM ALGORITHM
      t = (a[a_L] - '0') + (b[a_L] - '0') + C;
      C = (t > 9 ? 1 : 0);
      if (C) { t -= 10; t += '0'; r[a_L] = t; } else { t += '0'; r[a_L] = t; } 
    }
    if (C) { z[0] = '1'; z[1] = '\0'; } else z[0] = '\0'; 
    strcat(z, r);
...
char* addfs(const char* op1, const char* op2, int check) { //DEFAULT ARGS: (int check = 1) (check=0 => +10.00%)
    /**
    ...
    *   RUN-TIME ERROR CHECK: YES
    *   NEED: free()
    *   CALL: addis lstripf0 rstripf0 strpads0 is_strfmt_float raise freemem reset
    *   TIME: [186us-...]
    *   LIMIT: RAM MEMORY
    *
    *   EXAMPLES:
    ...
    *   \endcode
    */
  i32 DOT, a_L, b_L, aF_len, bF_len, sum_L;                //VARIABLES
  ...
  a_L = (i32)strlen(op1); b_L = (i32)strlen(op2);
  i32 DIM = (a_L + b_L + 8) * sizeof(char);                          //1 => NULL, 1 => CARRY, 1 => DOT 
  a  = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();        //RAM DYNAMIC ALLOCATION
  b  = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();       //RAM DYNAMIC ALLOCATION       
  RE = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();      //RESULT
  
  aI = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();    //SPLIT FLOATING POINT OPERAND STRING
  aF = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();   //SPLIT FLOATING POINT OPERAND STRING
  bI = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();  //SPLIT FLOATING POINT OPERAND STRING
  bF = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset(); //SPLIT FLOATING POINT OPERAND STRING
  strcpy(a, op1); strcpy(b, op2); //CLEARING ZEROS (020.00100 => 20.001)
  if (a[0] == '0') { lstripf0(a); a_L = (i32)strlen(a); } if (a[a_L - 1] == '0') { rstripf0(a); a_L = (i32)strlen(a); }
  if (b[0] == '0') { lstripf0(b); b_L = (i32)strlen(b); } if (b[b_L - 1] == '0') { rstripf0(b); b_L = (i32)strlen(b); }
  ...
  aF_len = (i32)strlen(aF); bF_len = (i32)strlen(bF);
  ...
  sum_L = (i32) strlen(SUM);
  RE[0] = '\0'; //"" => VOID STRING
  i32 ZERO = aF_len - sum_L;
...
/// NUM OUT-LINE /// ABSOLUTE ARBITRARY-PRECISION INTEGER NUMERIC STRING MULTIPLICATION (UNSIGNED NEED: free()), CODE: char *R = mulis_school("3141592654", "2718281828"); print(R, " => mul\n"); free(R); //8539734222346491512
char* mulis_school(const char* op1, const char* op2, int check) { //DEFAULT ARGS: (int check = 1) (check=0 => +1.3%)
    /**
    ...
    *   RUN-TIME ERROR CHECK: YES
    *   NEED: free()
    *   CALL: addis is_strfmt_int stripi0 str_rev I32str strpads0 raise freemem reset
    *   TIME: [1215us-...]
    *   LIMIT: RAM MEMORY
    *
    *   EXAMPLES:
    *
    *       char *R = mulis_school("3141592654", "2718281828");        print(R, " => mul\n"); free(R);   //8539734222346491512
    *       char *R = mulis_school("3141592654", "2718281828", false); print(R, " => mul\n"); free(R);  //8539734222346491512 (+1.3%)
    *       char *R = mulis_school("0.0", "3");                        print(R, " => mul\n"); free(R); //ERROR ARGUMENT VALUE => mulis: [0.0]
    *
    *   \endcode
    */
    i32 a_L, b_L, k, DIM; //VARIABLES
    I32 M, C = 0;        //MULTIPLY AND CARRY - VARIABLEs
    ...
    a_L = (i32)strlen(op1); b_L = (i32)strlen(op2);
    DIM = (a_L > b_L ? a_L : b_L) * sizeof(char) + 16; //1 => NULL
    a = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();  //RAM DYNAMIC ALLOCATION
    b = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    strcpy(a, op1); if (a[0] == '0') { stripi0(a); a_L = (i32)strlen(a); } //REMOVE NO VALUE ZEROs (003 => 3)
    strcpy(b, op2); if (b[0] == '0') { stripi0(b); b_L = (i32)strlen(b); }
    ...
    ar = (char*)malloc(DIM);     if (freemem() < RAM_SYS) reset();     //RAM DYNAMIC ALLOCATION
    br = (char*)malloc(DIM);     if (freemem() < RAM_SYS) reset();    //RAM DYNAMIC ALLOCATION
    V = (char*)malloc(DIM * 2);  if (freemem() < RAM_SYS) reset();   //RAM DYNAMIC ALLOCATION
    Vr = (char*)malloc(DIM * 2); if (freemem() < RAM_SYS) reset();  //RAM DYNAMIC ALLOCATION
    T = (char*)malloc(DIM * 2);  if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    ...
    for (i32 j = 0; j < b_L; j++) { //SIMPLE SCHOOL MULTIPLICATION ALGORITHM
        ...
        for (i32 i = 0; i < a_L; i++) {
            ...
            else { MS[0] = M + '0'; MS[1] = '\0'; strcat(Vr, MS); }
            if (C && i == (a_L - 1)) { MS[0] = C + '0'; MS[1] = '\0'; strcat(Vr, MS); }
        }
        strcpy(V, str_rev(Vr));
...
char* mulis(const char* op1, const char* op2, int check) { //DEFAULT ARGs: (int check = 1) (check=0 => +5.00%)
    /**
    *   \brief  ABSOLUTE ARBITRARY-PRECISION NUMERIC INTEGER STRING MULTIPLICATION (UNSIGNED)
    *   \param  op1   MULTIPLICAND NUMERIC INTEGER STRING
    *   \param  op2   MULTIPLIER   NUMERIC INTEGER STRING
    ...
    *   RUN-TIME ERROR CHECK: YES
    *   NEED: free()
    *   CALL: is_strfmt_int stripi0 str_rev raise freemem reset
    *   TIME: [161us-...]
    *   LIMIT: RAM MEMORY
    *
    *   EXAMPLES:
    ...
    *   \endcode
    */
    i32 a_L, b_L, dim, i, za = 0, zb = 0, Z = 0; //VARIABLES
    // I32 C = 0;           //MULTIPLY AND CARRY 
    ...
    a_L = (i32)strlen(op1); b_L = (i32)strlen(op2);
    a = (char*)malloc(a_L + 4); if (freemem() < RAM_SYS) reset();  //RAM DYNAMIC ALLOCATION
    b = (char*)malloc(b_L + 4); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    strcpy(a, op1); if (a[0] == '0') { stripi0(a); a_L = (i32)strlen(a); } //REMOVE NO VALUE ZEROs (003 => 3)
    strcpy(b, op2); if (b[0] == '0') { stripi0(b); b_L = (i32)strlen(b); }
    if (a[0] == '0') { free(b); return a; }               //op1 => ZERO return FACTOR1 (0 * 3)
    if (b[0] == '0') { free(a); return b; }              //op2 => ZERO return FACTOR2 (3 * 0)
    ...
    if(b_L == 1 && b[0] == '1') { free(b); return a; } //op2 => ONE return FACTOR1 (3 * 1)
    i32 c = a_L - 1; 
    if (a[c] == '0') { Z = 1; for (; a[c] == '0'; c--) za++; rstripc(a, '0'); a_L = (i32)strlen(a); }
    c = b_L - 1; 
    if (b[c] == '0') { Z = 1; for (; b[c] == '0'; c--) zb++; rstripc(b, '0'); b_L = (i32)strlen(b); }
    int zab = za + zb;     //ZEROs COUNT
    char* PRO = (char*)malloc((a_L + b_L + zab + 1) * sizeof(char)); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    dim = (a_L + b_L);
    PRO[0] = '0';       //INITIAL ZERO DIGIT FOR LEFT PADDING
    PRO[dim--] = '\0'; //FINAL NULL STRING TERMINATOR
    int m = a_L - 1, n = b_L - 1, carry = 0;
    for(i = 0; i <= m + n || carry; ++i) { //SANKET GODE MULTIPLICATION ALGORITHM
        for (int j = (0 < i - n ? i - n : 0); j <= (i < m ? i : m); ++j) 
...
/// NUM OUT-LINE /// CLEAR INTEGER NUMBER STRING s LEFT PART FROM ZERO CHARACTER (s STRING MODIFIED), CODE: char s[] = "03"; stripi0(s); print(s, "\n"); //3
char* stripi0(char* s) {
    /**
    ...
    */
    I32 qty = 0;
    i32 L_s = (i32)strlen(s);
    while (s[qty] == '0') qty++;
    if (qty) { I32 i = 0; while (s[i + qty]) { s[i] = s[i + qty]; i++; } s[i] = '\0'; }
...
char* strpads(const char* s, i32 qty) {
    /**
    ...
    */
    char* str;
    qty = qty < 0 ? 0 : qty;
    str = (char*)malloc((qty * strlen(s) + 1) * sizeof(char)); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    str[0] = '\0';
    for (i32 i = 0; i < qty; i++) strcat(str, s);
    return str;
}
...
char* strpads0(i32 qty) {
    ...
    */
    ...
    i32 i;
    qty = qty < 0 ? 0 : qty;
    str = (char*)malloc((qty + 1) * sizeof(char)); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
...
char* subis(const char* op1, const char* op2, int check) { //DEFAULT ARGs: (int check = 1) (check=0 => +30%)
    ...
    *   \endcode
    */
    char* a, * b, * z, * r; //RAM POINTERS VARIABLEs
    int SIGN = 0;          //SIGN DEFAULT TO ZERO
    int C = 0;            //CARRY DEFAULT TO ZERO => BORROW
    i32 DIM, offset, t;  //GENERAL PURPOSE
    i32 a_L = (i32)strlen(op1), b_L = (i32)strlen(op2);
    if (check) {       //ARGs VALIDATION
    ...
    DIM = (a_L > b_L ?a_L:b_L) * sizeof(char) + 4;    //1 => NULL, 1 => SIGN
    a = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();    //RAM DYNAMIC ALLOCATION
    b = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();   //RAM DYNAMIC ALLOCATION
    z = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();  //RAM DYNAMIC ALLOCATION
    r = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    ...
    free(a); free(b); 
    if (SIGN) { strcpy(z, "-"); strcat(z, r[0]=='0' ? stripi0(r) : r); //1000-1003=0003 => -3 (CLEAR ZEROs)
        free(r); return z; } //3-4=-1
    free(z);
...
char* subfs(const char* op1, const char* op2, int check) { //DEFAULT ARGs: (int check = 1) (check = 0 => +11.00%)
    ...
    *   RUN-TIME ERROR CHECK: YES
    *   NEED: free()
    *   CALL: subis is_strfmt_float lstripf0 rstripf0 strpads0 raise freemem reset
    *   TIME: [181us-...]
    *   LIMIT: RAM MEMORY
    *
    *   EXAMPLES:
    ...
    *   \endcode
    */
    i32 DOT, SIGN, aF_len, bF_len, a_L, b_L, DIM;                //VARIABLES
    ...
    a  = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();       //RAM DYNAMIC ALLOCATION
    b  = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();      //RAM DYNAMIC ALLOCATION
    aI = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();     //RAM DYNAMIC ALLOCATION
    aF = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();    //RAM DYNAMIC ALLOCATION
    bI = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();   //RAM DYNAMIC ALLOCATION
    ...
    if (!strcmp(a, b)) { //OPERANDS EQUAL THEMSELVES, a==b => '0.0' DIFFERENCE
        ...
        free(a); free(b); free(aF); free(bI); free(bF); free(RE);
        return aI; }
    strcpy(aI, strtok(a, ".")); strcpy(aF, strtok(0, ".")); aF_len = (i32)strlen(aF);
    strcpy(bI, strtok(b, ".")); strcpy(bF, strtok(0, ".")); bF_len = (i32)strlen(bF);
    ...
    DOT = (i32)strlen(RAM_X) - (bF_len > aF_len ? bF_len : aF_len);
    ...
    if (RE[0] == '.') {
        ...
        else strcpy(aI, RE); }                        //5.012-0.12 =  4.892
...
char* str_rev(char* s) {
    ...
    i32 s_len = (i32)strlen(s);
    i32 s_mid = s_len / 2;
    for (i32 i = 0; i < s_mid; i++) { T = s[i]; pos = s_len - i - 1; s[i] = s[pos]; s[pos] = T; }
    return s;
}
...
const char* I32str(I32 num, int base) { //DEFAULT ARGs: (int base = 10) 
    ...
    return ultoa(num, digits, base);  //RETURN ONLY POSITIVE WITHOUT SIGN (sizeof(long int) = 4)
}
...
I64 I64int(const char* n, int base) { //DEFAULT ARGs: (int base = 10)
    I64 N = 0;
    switch (base) { case 16: break; case 10: break; case 8: break; case 2: break; default: base = 10; }
    if(base > 10)
    ...
    else for (int i = 0; i < strlen(n); i++) { N *= base; N += n[i] - '0'; } //DIGIT TO VALUE 
    return N;
}
/// NUM OUT-LINE /// CONVERT UNSIGNED 32-BIT INTEGER STRING TO UNSIGNED 32-BIT INTEGER, CODE: print(I32int("400000000") + 1, "\n"); //400000001
...
/// NUM OUT-LINE /// CONVERT SIGNED 32-BIT INTEGER STRING TO SIGNED 32-BIT INTEGER, CODE: print(i32int("-2100000000") + 1, "\n"); //-2099999999
i32 i32int(const char* s, int base, int check) { //DEFAULT ARGs: (int base = 10, int check = 1);
    //*   \param  check PARAMETER VALIDATION AND EXCEPTIONAL RESULT AND ERROR
    if (check) if (OFI31(s)) { raise("ARGUMENT VALUE => i32int", s); return 0; } //32-BIT SIGNED INTEGER OVERFLOW
    return (i32)strtoll(s, NULL, base); 
}
/// NUM OUT-LINE /// CHECK OVERFLOW FOR 32 BIT UNSIGNED INTEGER STRING (0-4294967295 RANGE), CODE: const char* n = "4294967295"; int t = OFI32(n); print(t ? "OVERFLOW" : "SUCCESS", "\n"); //"SUCCESS"
int OFI32(const char* stri) {
    /**
    *   \brief  CHECK OVERFLOW FOR 32-BIT UNSIGNED INTEGER STRING
    *   \param  stri STRING
    *   \return int
    *   \code
    ...
    *   EXAMPLES:
    *
    *       const char* n = "4294967295"; int t = OFI32(n); print(t ? "OVERFLOW" : "SUCCESS", "\n");   //"SUCCESS"
    ...
    */
    char MAX[] = "4294967295"; 
    int  stri_L = (int)strlen(stri); 
    for (int i = 0; i < stri_L; i++) if (!isdigit(stri[i])) return 1;
...
int OFI31(const char* n) {
    ...
    */
        ...
        i32 s_L = (i32)strlen(s);
        for (int i = 0; i < s_L; i++) if (!isdigit(s[i])) return 1;            //NOT VALID
...
char* mulfs(const char* op1, const char* op2, int check) { //DEFAULT ARGs: (int check = 1) 
    ...
    *   RUN-TIME ERROR CHECK: YES
    *   NEED: free()
    *   CALL: mulis is_strfmt_float lstripf0 rstripf0 strpads0 raise freemem reset
    *   TIME: [547us-...]
    *   LIMIT: RAM MEMORY
    *
    *   EXAMPLES:
    ...
    *   \endcode
    */
    i32 DOT, M_L, aF_len, bF_len, Z, a_L, b_L, DIM;       //VARIABLEs
    ...
    M_L = (i32)strlen(RAM_RE); Z = M_L - DOT;
    if (Z > 0) {
...
char* invfs(const char* div, i32 d, int check) { //DEFAULT ARGUMENTS: (i32 d = 30, int check = 1)
    /**
    ...
    *   \param  d   DECIMAL DIGITS i32
    *   \param  check PARAMETER VALIDATION AND EXCEPTIONAL RESULT AND ERROR
    *   \return R s QUOTIENT FLOATING-POINT STRING
    *   \code
    *   RUN-TIME ERROR CHECK: YES
    *   NEED: free()
    *   CALL: divfs is_strfmt_float lstripf0 rstripf0 raise freemem reset
    *   TIME: [11ms-...]
    *   LIMIT: RAM MEMORY
    *
    *   EXAMPLES:
...
/// NUM OUT-LINE /// ABSOLUTE FLOATING-POINT STRING DIVISION USEFUL BY DIVISOR INVERSE MULTIPLICATION (UNSIGNED, NEED: free()), CODE: char* R = divfs("3.141592654", "2.718281828", 10, 0); Serial.println(R); free(R); //1.1557273501 (check=0 -> +4.91%)  
char* divfs(const char* op1, const char* op2, i32 d, int check) { //ARGUMENTS DEFAULT: (i32 d = 30, int check = 1) (check=0 -> +4.91%)
    /**
    *   \brief  ABSOLUTE FLOATING-POINT STRING DIVISION USEFUL BY DIVISOR INVERSE MULTIPLICATION (UNSIGNED, NEED: free())
    ...
    *   ERROR CHECK: YES
    *   NEED: free()
    *   CALL: divis is_strfmt_float lstripf0 rstripf0 strpads0 raise freemem reset
    *   TIME: [4.4ms-...]
    *   LIMIT: RAM MEMORY
    *
    *   EXAMPLES:
    *
    *       char* R = divfs("3.141592654", "2.718281828", 10, 0); print(R); free(R);   //1.1557273501 (check=0 -> +4.91>)
    ...
    */
    ...
    i32 a_L = (i32)strlen(op1); i32 b_L = (i32)strlen(op2);
    i32 DIM = ((a_L > b_L ? a_L : b_L) + 16) * sizeof(char); //1 => NULL, 1 => DOT
    ...
    while (a[a_L - 1] == '0' && a[a_L - 2] =='.' && b[b_L - 1] == '0' && b[b_L - 2] == '.') { //12300.0:100.0=123:1=123
        ...
        c = b_L - 3;
        if (b[c] == '0') { for (; b[c] == '0'; c--) zb++; a_L = (i32)strlen(a); b_L = (i32)strlen(b); }
        else break;
        int Z = (za < zb ? za : zb); //ZEROs COUNT TO CLEAR
    ...
    if (check) {
        ...
        else if (!strcmp(a, "0.0"))   { free(b); return a; }                       //DIVIDEND EQUALS ZERO
        else if (!(strcmp(b, "1.0"))) { free(b); return a; }                      //DIVISION BY ONE
        else if (!(strcmp(b, a)))     { free(b); strcpy(a, "1.0"); return a; }   //DIVISOR EQUALS DIVIDEND
    }
    char* R  = (char*)malloc(DIM + d); if (freemem() < RAM_SYS) reset();     //RAM DYNAMIC ALLOCATION
    char* aI = (char*)malloc(DIM);     if (freemem() < RAM_SYS) reset();    //RAM DYNAMIC ALLOCATION
    char* aF = (char*)malloc(DIM);     if (freemem() < RAM_SYS) reset();   //RAM DYNAMIC ALLOCATION
    char* bI = (char*)malloc(DIM);     if (freemem() < RAM_SYS) reset();  //RAM DYNAMIC ALLOCATION
    char *bF = (char*)malloc(DIM);     if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    i32 aF_len; i32 bF_len;
    strcpy(aI, strtok(a, "."));
    strcpy(aF, strtok(0, ".")); aF_len = (i32)strlen(aF);
    strcpy(bI, strtok(b, "."));
    strcpy(bF, strtok(0, ".")); bF_len = (i32)strlen(bF);
    char* as, * bs, * quotient, * remainder; //RAM POINTERS
    if (aF_len > bF_len) { //3.14:2.9=1.082758 d=6
        I32 ze = aF_len - bF_len;
        char* st = strpads0(ze);
        int st_L = (int)strlen(st); 
        DIM += st_L;
        as = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();          //RAM DYNAMIC ALLOCATION
        bs = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();         //RAM DYNAMIC ALLOCATION
        quotient = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();  //RAM DYNAMIC ALLOCATION
        remainder = (char*)malloc(DIM);if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
        strcpy(as, aI); strcat(as, aF);
        strcpy(bs, bI); strcat(bs, bF);
        strcat(bs, st); free(st);
    }
    else { //2.9:3.14=0.923566 d=6
        I32 ze = bF_len - aF_len;
        ...
        } //987654321987654321.0:12345678.0=80000006641.000544563 d=10
        else { 
            char* st = strpads0(ze); 
            int st_L = (int)strlen(st); 
            DIM += st_L;
    ...
    for (I32 k = d, rem_L; k > 0; k--) { //COMPUTING DECIMALS DIGITs (d != 0)
        rem_L = strlen(remainder); remainder[rem_L] = '0'; remainder[rem_L+1] = '\0'; //10x remainder
...
Division* divis(const char* n, const char* div, int check) { //PROTOTYPE ARGs: (int check = 1) (+5.80%)
    ...
    *   CALL: divfs mulfs mulis addis subis strint_cmp is_strfmt_int I64str I64int stripi0 raise freemem reset
    *   TIME: [468us-...]
    *   LIMIT: RAM MEMORY 
    *
    *   EXAMPLES:
    ...
    *   \endcode
    */
    static Division division; //RESULT: QUOTIENT AND REMAINDER -VARs
    char* R, * diff;         //TEMPORARY
    if (check) {
        if (!is_strfmt_int(n))   { raise("ARGUMENT VALUE => divis", n);   division.quotient = division.remainder = NULL; 
            return &division; }
        if (!is_strfmt_int(div)) { raise("ARGUMENT VALUE => divis", div); division.quotient = division.remainder = NULL;
            return &division; }
    }
    i32 a_L = (i32)strlen(n); i32 b_L = (i32)strlen(div);
    i32 DIM = ((a_L > b_L ? a_L : b_L) + 16) * sizeof(char); //1=>'-' + 1=>NULL + 2=>".0" 
    char* rem  = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();   //RAM DYNAMIC ALLOCATION
    char* rem2 = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();  //RAM DYNAMIC ALLOCATION
    char* DIV  = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    strcpy(rem, n);    strcpy(DIV, div); 
    if (rem[0] == '0') { stripi0(rem); a_L = (i32)strlen(rem); }     //REMOVE NO VALUE ZEROs (003 => 3)
    if (DIV[0] == '0') { stripi0(DIV); b_L = (i32)strlen(DIV); } 
    if (check) {
    ...
    if (a_L - b_L > 2) {         //TOO LONG TIME SUBTRACTION LOOP DIVISION
        if(b_L < 19) {
            char* result = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
        ...
        strcat(rem, ".0"); strcat(DIV, ".0");   //LONG DIVISION BY DIVISOR INVERSE MULTIPLICATION
        char* DIV_inv = divfs("1.0", DIV, DIM - 16, false); //1:DIV (INVERSE)
        R = mulfs(rem, DIV_inv, false); free(DIV_inv);
        i32 i = 0; while (R[i] != '.') i++; R[i] = '\0'; //NUMBER INTEGER PART
    ...
    I32 Q = 0;        //RESULT: QUOTIENT BY LOOP SUBTRACTION
    ...
    division.quotient = strcpy(rem, I32str(Q)); free(DIV);
    return &division;
}
...
/// NUM OUT-LINE /// ERROR HANDLER: SET Error = 1, CODE: error_set(); print(error() ? "RUN TIME ERROR: YES\n":"RUN TIME ERROR: NO\n"); //RUN TIME ERROR: YES
int error_set() { 
  return Error = 1;
}
/// NUM OUT-LINE /// ERROR HANDLER: SET Error = 0, CODE: error_clear(); print(error() ? "RUN TIME ERROR: YES\n":"RUN TIME ERROR: NO\n"); //RUN TIME ERROR: NO
int error_clear() { 
  return Error = 0;
}
...
int strfloat_cmp(const char* op1, const char* op2) { 
    ...
    *   ERROR CHECK: YES
    *   NEED: NONE
    *   CALL: is_strfmt_float stripi0 strpads0 freemem reset
    *   TIME: [100us-...]
    *   LIMIT: RAM MEMORY
    *
    *   EXAMPLES:
    ...
    */
    i32 DOT;
    i32 a_L = (i32)strlen(op1); i32 b_L = (i32)strlen(op2);
    i32 DIM = (a_L > b_L ? a_L : b_L) + 1 + 1; //1 => NULL, 1 => DOT
    ...
    char* aI = (char*)malloc(DIM * 2 * sizeof(char)); if (freemem() < RAM_SYS) reset();    //RAM DYNAMIC ALLOCATION
    char* aF = (char*)malloc(DIM * 2 * sizeof(char)); if (freemem() < RAM_SYS) reset();   //RAM DYNAMIC ALLOCATION
    char* bI = (char*)malloc(DIM * 2 * sizeof(char)); if (freemem() < RAM_SYS) reset();  //RAM DYNAMIC ALLOCATION
    char* bF = (char*)malloc(DIM * 2 * sizeof(char)); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    strcpy(a, op1); strcpy(b, op2); 
    i32 aF_len; i32 bF_len; //ARGS a and b MUST BE FLOATING POINT FORMAT!
    if (!is_strfmt_float(a)) { raise("ARGUMENT VALUE => strfloat_cmp", op1); return 0; }
    if (!is_strfmt_float(b)) { raise("ARGUMENT VALUE => strfloat_cmp", op2); return 0; }
    strcpy(aI, strtok(a, "."));
    strcpy(aF, strtok(0, ".")); aF_len = strlen(aF);
    strcpy(bI, strtok(b, "."));
    strcpy(bF, strtok(0, ".")); bF_len = strlen(bF);
    if (aF_len < bF_len) {          //"3.1"+"0.94"="4.04"
        DOT = bF_len - aF_len;
        strcpy(a, aI); strcat(a, aF); char* RAM_pad = strpads0(DOT); strcat(a, RAM_pad); free(RAM_pad);
        strcpy(b, bI); strcat(b, bF);
    }
    ...
    i32 op1_len = (i32)strlen(a);
    i32 op2_len = (i32)strlen(b);
    int COMPARE;
    if (op1_len > op2_len) {
        COMPARE = 1;
...
int strint_cmp(const char* op1, const char* op2) {
    ...
    *   ERROR CHECK: YES
    *   NEED: NONE
    *   CALL: is_strfmt_int stripi0 freemem reset
    *   TIME: [28us-...]
    *   LIMIT: NONE
    *
    *   EXAMPLES:
    ...
    */
    ...
    i32 a_L = (i32)strlen(op1); i32 b_L = (i32)strlen(op2);
    i32 DIM = (a_L > b_L ? a_L : b_L) + 4; //1 => NULL, 1 => DOT
    char* a = (char*)malloc(DIM * 2 * sizeof(char)); if (freemem() < RAM_SYS) reset();  //RAM DYNAMIC ALLOCATION
    char* b = (char*)malloc(DIM * 2 * sizeof(char)); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    strcpy(a, op1); strcpy(b, op2); //op1 => a op2 => b
    if(a[0] == '0') stripi0(a); 
...
char* lstrip(char* s, const char* cs) {
    ...
    *   \endcode
    */
    int s_LI = (int)(i32)strlen(s);
    int s_LF;
    while (1) {
        for (int i = 0; i < (i32)strlen(cs); i++) lstripc(s, cs[i]);
        s_LF = (int)(i32)strlen(s);
        if (s_LI == s_LF) break;
        s_LI = s_LF;
    }
    return s;
...
char* rstrip(char* s, const char* cs) {
    ...
    */
    ...
    int s_LF;
    while (1) {
        for (int i = 0; i < (i32)strlen(cs); i++) rstripc(s, cs[i]);
        s_LF = (int)(i32)strlen(s);
        if (s_LI == s_LF) break;
        s_LI = s_LF;
    }
...
/// NUM OUT-LINE /// REMOVE ALL OCCURRENCES OF A SPECIFIC CHARACTER FROM A STRING, CODE: char s[] = "123_456_789"; char* S = rm_c(s, '_'); print(S, "\n"); //123456789
...
char* lstripc(char* s, const char ch) {
    ...
    */
    i32 qty = 0;
    while (s[qty] == ch) qty++;
    if (qty) { i32 i = 0; while (s[i + qty]) { s[i] = s[i + qty]; i++; } s[i ? i : 1] = '\0'; }
    return s;
}
...
/// NUM OUT-LINE /// CHECK IF A NUMERIC STRING IS EXPONENTIAL FORMAT, CODE: const char* n = "1.0e-2", *boo = is_strfmt_exp(n); print(boo ? boo : "false", "\n");  //1.0e-2
const char* is_strfmt_exp(const char* sn) {
    ...
    *   RUN-TIME ERROR CHECK: YES
    *   NEED: NONE
    *   CALL: is_strfmt_int freemem reset
    *   TIME: [91us-...]
    *   LIMIT: RAM MEMORY
    *
    *   EXAMPLES:
    ...
    */
    ...
    char* bf0 = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();  //BASE INTEGER   PART 
    char* bf1 = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset(); //BASE FRACTIONAL PART 
    strcpy(r1, sn);
    int e = 0; //COUNTER 'e' CHARACTER
    for (int i = 0; i < strlen(r1); i++) { if (toupper(r1[i]) == 'E') e++; r1[i] = toupper(r1[i]); }
...
*/
    static char* P[2];
    i32 DIM = (i32)strlen(s) + 16; //1 NULL
    char* S =  (char*)malloc(DIM * sizeof(char)); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    strcpy(S, s);
...
char* lower(const char* s) {
    i32 L = (i32)strlen(s);
    char* m = (char*)malloc((L + 1) * sizeof(char)); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    m[L] = '\0'; //NULL STRING TERMINATOR
    for (i32 i = 0; i < L; i++) m[i] = tolower(s[i]);
    return m;
}
...
char* upper(const char* s) {
        ...
        char* m = (char*)malloc((L + 1) * sizeof(char)); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
        m[L] = '\0'; //NULL STRING TERMINATOR
        for (i32 i = 0; i < L; i++) m[i] = toupper(s[i]);
        return m;
}
/// NUM OUT-LINE /// CONVERT A NUMERIC (BOTH INTEGER AND FLOATING-POINT) STRING TO EXPONENTIAL SCIENTIFIC NOTATION STRING (SIGNED) -NEED: free(), CODE: char* p = num2exp("-10_000"); print(p, "\n"); free(p); //-1.0e4
char* num2exp(const char* a) {
    ...
    *   RUN-TIME ERROR CHECK: YES
    *   NEED: free()
    *   CALL: is_strfmt_int is_strfmt_float stripi0 rstripf0 stripf0 I32str freemem reset 
    *   TIME: [49us-...]
    *   LIMIT: RAM MEMORY
    *
    *   EXAMPLES:
    ...
    */
    ...
    static int NEG;
    static i32 e;
    i32 a_L = (i32)strlen(a); 
    i32 DIM = a_L + 16; //NULL, '.', '-', '+', e- 
    char* A  = (char*)malloc(DIM * sizeof(char)); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    char* TM = (char*)malloc(DIM * sizeof(char)); if (freemem() < RAM_SYS) reset();//RAM DYNAMIC ALLOCATION
    strcpy(A, a);
    ...
    if (is_strfmt_int(A)) { //INTEGER FORMAT: 34 => 3.4e1  -1000 => -1.0e3  112500 => 1.125e5
        if(A[0] == '0') stripi0(A); //CLEAR ZEROs
        ...
        strcpy(A, NEG ? "-" : ""); strcat(A, TM); strcat(A, "e"); strcat(A, I32str(e));
        free(TM);
        return A; 
    }
        ...
        if (!strcmp(p[0], "0")) { //0.___
            //DEBUG: print(p[0], " "); print(p[1], "\n");
            i32 L_F0 = (i32)strlen(p[1]);
            strcpy(TM, lstripf0(p[1]));
            i32 L_F = (i32)strlen(TM);
            e = L_F0 - L_F + 1;
            if (L_F == 1) {
                strcpy(A, NEG ? "-" : ""); strcat(A, TM); 
                if (!strcmp(TM, "0")) { strcat(A, ".0e0"); }
                else { strcat(A, ".0e-"); strcat(A, I32str(e)); }
                free(TM); free(p[0]);
                return A; //-0.01 => -1.0e-2  -0.000001 => -1.0e-6
            }
            strcpy(A, NEG ? "-" : ""); strncat(A, TM, 1); strcat(A, "."); 
            strcat(A, TM + 1); strcat(A, "e-"); strcat(A, I32str(e));
            free(TM); free(p[0]);
...
/// NUM OUT-LINE /// UNSIGNED INTEGER SUPER POWER, CODE: char *R = pwri(2, 128); print(R, "\n"); free(R); //340282366920938463463374607431768211456
char* pwri(I32 b, I32 e) {
    /**
    ...
    *   RUN-TIME ERROR CHECK: YES
    *   NEED: free()
    *   CALL: I32str i32str raise freemem reset
    *   TIME: [4ms-...]
    *   LIMIT: RAM MEMORY
    *
    *   EXAMPLES:
    ...
    *   \endcode
    */
    if (!b && !e) { raise("IMPOSSIBLE OPERATION => pwri", i32str(b)); return NULL; } //0^0
    I32 SIZE = 0, T = b;
    I32 C = 0, P, j;
    i32 i = (i32)strlen(I32str(b)) * e * sizeof(char);
    char* s = (char*)malloc(i + 4);            //1 => NULL (".0" SUFFIX)
    if (freemem() < RAM_SYS) reset();         //RAM DYNAMIC ALLOCATION
    if (!e) { strcpy(s, "1"); return s; }    //b^0 = 1
    I32* R = (I32*)malloc(i * sizeof(I32)); //RAM DYNAMIC ALLOCATION
    if (freemem() < RAM_SYS) reset();
...
NUM& pwrf_signed(NUM *b, i32 e) {
    ...
    *   RUN-TIME ERROR CHECK: YES
    *   NEED: NONE
    *   CALL: i32str raise reset freemem  
    *   TIME: [4ms-...]
    *   LIMIT: RAM MEMORY
    *
    *   EXAMPLES:
    ...
    */
    ...
    E = b->E * e;
    Es = i32str(E);
    P10 = (char*)malloc(((i32)strlen(Es) + 16) * sizeof(char)); //"1", 'e', '-', NULL, ".0"
    if (freemem() < RAM_SYS) reset();                          //RAM DYNAMIC ALLOCATION
...
/// NUM OUT-LINE /// CONVERT EXPONENTIAL NOTATION NUM TO A STANDARD NUMBER STRING (SIGNED) -NEED: free(), CODE: NUM a("12345678901234567891.0e-10"); char* RAM = exp2num(a); print(RAM ? RAM : "FAILURE", "\n"); free(RAM); //1234567890.1234567891
char* exp2num(NUM &x) {
    /**
    ...
    *   RUN-TIME ERROR CHECK: YES
    *   NEED: free()
    *   CALL: split stripi0 lstripf0 rstripf0 strpads strpads0 freemem reset
    *   TIME: [87us-...]
    *   LIMIT: RAM MEMORY
    *
    *   EXAMPLES:
    ...
    *   \endcode
    */
    static int NEG; NEG = x.S;
    static i32 EXP, L_bf0, L_bf1, DOT, DIM;
    static char* r1, * r2, *bf0, *bf1, *pad;
    static char **m; //TWO POINTER ARRAY
    EXP = x.E;
    DIM = strlen(x.C) + 16 + (x.E<0?-x.E:x.E);
    r1 = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    r2 = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();//RAM DYNAMIC ALLOCATION
    m = split(x.C, ".");
    bf0 = m[0]; bf1 = m[1]; 
    L_bf0 = strlen(bf0); L_bf1 = strlen(bf1);
    if (x.E >= 0) {                        //POSITIVE EXPONENT OR ZERO
        ...
        for (int i = 0; i < strlen(r2); i++) //FLOATING-POINT RESULT
            if (r2[i] == '.') {
        ...
        //INTEGER RESULT
        if(r2[0] == '0') stripi0(r2); //CLEAR ZEROS
...
/// NUM OUT-LINE /// CONVERT EXPONENTIAL NOTATION STRING TO NUMBER STRING (SIGNED) NEED: free(), CODE: char* RAM = exp2str("12345678901234567891.0e-10"); print(RAM, "\n"); free(RAM); //1234567890.1234567891
...
NUM& mod(NUM &op1, NUM &op2) {
    ...
    R = exp2num(Q); 
    NIF = split(R, "."); 
    nif = (char*)malloc(((i32)strlen(NIF[0]) + 1) * sizeof(char)); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    strcpy(nif, NIF[0]); free(NIF[0]); free(R);
    if (!strcmp(nif, "0") || !strcmp(nif, "-0")) { free(nif); return result = op1; }
    if (op1 == op2) { result = (int)0; free(nif);  return result; }
    as = (char*)malloc(((i32)strlen(nif) + 3) * sizeof(char)); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    strcpy(as, nif); strcat(as, ".0"); q = as; free(as); free(nif);
    return result = op1 - q * op2;
}
...
char* trunks(const char* n, i32 d) {
    /**
    ...
    *   RUN-TIME ERROR CHECK: YES
    *   NEED: free()
    *   CALL: is_strfmt_float stripf0 rstripf0 strpads0 raise freemem reset
    *   TIME: [143us-...]
    *   LIMIT: RAM MEMORY
    *
    *   EXAMPLES:
    ...
    */
    ...
    i32 n_len = (i32)strlen(n);
    i32 DIM  = (n_len + 1 + 1) * sizeof(char); //NULL TERMINATOR AND SIGN PLACE
    char* N  = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();   //RAM DYNAMIC ALLOCATION
    char* Ni = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();  //Ni => INTEGER PART
    char* Nf = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset(); // nF => FRACTIONAL PART
    strcpy(N, n); stripf0(N);
    strcpy(Ni, strtok(N, ".")); strcpy(Nf, strtok(0, "."));
    i32 Ni_len = (i32)strlen(Ni);
    i32 diff;
    if (d < 0) {
        diff = Ni_len + d;
...
NUM& divf_signed(NUM* op1, NUM* op2, int dp) { //DEFAULT ARGs: (NUM*, NUM*, int dp = 32) 
    /**
...
NUM& divf(NUM* n, NUM* div, i32 dp) { //DEFAULT PARMs: (NUM*, NUM*, i32 dp = 30);
    /**
    ...
    *   RUN-TIME ERROR CHECK: YES
    *   NEED: NONE
    *   CALL: divfs i32str split freemem reset
    *   TIME: [13ms-...]
    *   LIMIT: RAM MEMORY
    *
    *   EXAMPLES:
    ...
    */
    ...
    free(R.C); R.C = p;
    q = split(p, ".");
    R.len_I = (i32)strlen(q[0]); R.len_F = (i32)strlen(q[1]); free(q[0]);
    free(R.CE); R.CE = (char*)malloc((R.len_I + R.len_F + (i32)strlen(Es) + 4) * sizeof(char)); //DOT, "-e", NULL
    if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
...
NUM& mulf(NUM* a, NUM* b) { 
    ...
    *   RUN-TIME ERROR CHECK: YES
    *   NEED: NONE
    *   CALL: mulfs i32str split freemem reset NUM
    *   TIME: [670us-...]
    *   LIMIT: RAM MEMORY
    *
    *   EXAMPLES:
    ...
    */
    ...
    Es = i32str(R.E);
    q = split(p, ".");
    R.len_I = strlen(q[0]); R.len_F = strlen(q[1]); free(q[0]);
    free(R.CE); R.CE = (char*)malloc((R.len_I + R.len_F + (i32)strlen(Es) + 4) * sizeof(char)); //DOT, "-e", NULL
    if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    strcpy(R.CE, R.C); strcat(R.CE, "e"); strcat(R.CE, Es);
    return R;
...
NUM& subf(NUM* a, NUM* b) {
    ...
    *   RUN-TIME ERROR CHECK: YES
    *   NEED: NONE 
    *   CALL: subfs expEQ i32str split freemem reset
    *   TIME: [318us-...]
    *   LIMIT: RAM MEMORY
    *
    *   EXAMPLES:
...
NUM& addf(NUM* a, NUM* b) {
    ...
    *   RUN-TIME ERROR CHECK: YES
    *   NEED: NONE
    *   CALL: addfs expEQ i32str split freemem reset
    *   TIME: [316us-...]
    *   LIMIT: RAM MEMORY
    *
    *   EXAMPLES:
...
/// NUM OUT-LINE /// EXPONENTIAL NOTATION EQUALIZATION (SIGNED) NEED: free(), CODE: char* R = expEQ("-7564322979.0e0", "1.0e10"); print(R, "\n"); free(R);  //-0.7564322979e10
char* expEQ(const char* a, const char* b) { 
    ...
    *   RUN-TIME ERROR CHECK: YES
    *   NEED: free()
    *   CALL: is_strfmt_exp rstripf0 stripf0 stripi0 i32str upper split strpads0 freemem reset 
    *   TIME: [494us-...]
    *   LIMIT: RAM MEMORY
    *
    *   EXAMPLES:
    ...
    */
    ...
    i32 a_L = (i32)strlen(a); i32 b_L = (i32)strlen(b);
    i32 DIM = (a_L > b_L ? a_L : b_L) + 16; //NULL, '.', '-', '+', e- (.0 SUFFIX)
    char* A = (char*)malloc(DIM * sizeof(char)); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    char* B = (char*)malloc(DIM * sizeof(char)); if (freemem() < RAM_SYS) reset();//RAM DYNAMIC ALLOCATION
    ...
    i32 A_E = (i32)atoll(p_A[1]); free(p_A[0]);        //A EXPONENT
    char** p_B = split(B, "E"); strcpy(B, p_B[0]);    //B BASE 
    i32 B_E = (i32)atoll(p_B[1]); free(p_B[0]);      //B EXPONENT
    i32 EXP = B_E;
    i32 OFFSET = B_E - A_E;                        //EXPONENT OFFSET
    if (!OFFSET) { strcpy(A, a); free(B); return A; } //-0.0123e6 1.0e6 => -0.0123e6 (UNCHANGED) (20240422)
    char* R = (char*)malloc(((OFFSET<0?-OFFSET:OFFSET)+DIM) * sizeof(char)); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    if      (A[0] == '-') { strcpy(A, A + 1); NEG = 1; } //SIGN
    else if (A[0] == '+') { strcpy(A, A + 1); NEG = 0; }
    else NEG = 0;
    char* AI, *AF;
    ...
    AF = (char*)malloc((strlen(AA[1])+1) * sizeof(char)); if (freemem() < RAM_SYS) reset(); //FRACTIONAL PART OF A
    strcpy(AI, AA[0]); strcpy(AF, AA[1]); free(AA[0]);
    if (AI[0] == '0' && !strcmp(AF, "0")) { strcpy(R, "0.0"); //&& AF[0] == '0' 
        free(A); free(B); free(AI); free(AF); return R; }    //-0.0e8 -123e6 => 0
    i32 len_AI = strlen(AI), len_AF = strlen(AF);
    i32 zero; 
    if (OFFSET <= 0 ) { //RIGHT SHIFT DOT 
        ...
        if (AI[0] == '0') {      //0. ...
            ...
            if(AF[0] == '0') stripi0(AF); //CLEAR ZEROS
            strcpy(R, NEG ? "-" : ""); strcat(R, AF); pad = strpads0(zero); strcat(R, pad); strcat(R, ".0");
            strcat(R, "e"); strcat(R, i32str(EXP));
            free(A); free(B); free(AI); free(AF); free(pad);
            return R;
...
char* bits(const char* n, int bin) { //DEFAULT ARGs: (int bin = true)
    Division* q;
    static char * B, * N, * H;
    static I32 i, j, H_len, B_len;
    N = (char*)malloc(((i32)strlen(n) + 1) * sizeof(char)); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    H = (char*)malloc(((i32)strlen(n) + 1) * sizeof(char)); if (freemem() < RAM_SYS) reset();//RAM DYNAMIC ALLOCATION
    strcpy(N, n);
    i = 0;
    do {
        j = 0;
    ...
    H_len = (I32)strlen(H);
    B_len = H_len * 4;
    B = (char*)malloc((B_len + 1) * sizeof(char)); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    i = 0; j = 0;
    do {
        switch ((char)H[j]) {
    ...
    free(N); free(H);
    if (B[0] == '0') stripi0(B);
    return B; //BINARY REPRESENTATION
}
...
NUM& POW(NUM &b, i32 e) {
    ...
    b = (e < 0 ? 1 / b : b);
    e = (e < 0 ? -e : e);
    if (b == (int)0) { q = (int)0; return q; }
    if (b == 1) { q = 1; return q; }
    bits = I32tobin(e);
    q = m = b;
    for (i = 1; i < (i32)strlen(bits); i++)
...
/// NUM OUT-LINE /// CALCULATOR MODE: POWER - HIGH SPEED, CODE: //int digits = 15; NUM b(1), n("1.0"); n.shift(digits); n.print_exp("\n"); b = 1 + div(b, n); NUM e = POW(b, n, digits * 2); e = e.trunk(digits - 1); print(e, "\n"); //2.71828182845904 (NATURAL CONSTANT BASE e)
NUM& POW(NUM &b, NUM& e, i32 d) {
    static NUM q, m;
    static int i;
    static char* bits;
    if (b == "0.0" && (!e || e < 0)) { raise("IMPOSSIBLE OPERATION => POW", b.CE); return q = 0; } //0^0 0^-e (e>0)
...
NUM& POW_MOD(NUM &b, i32 e, i32 n) { 
    /** b^e => DIVIDEND, n => DIVISOR, return R => REMAINDER */
    static NUM R;
    static i32 SIGN, i;
    static const char* bits;
    ...
    b = (e < 0 ? 1 / b : b);
    e = (e < 0 ? -e : e);
    if (!e) { b = 1; R = b % n; return R; }
    if (e == 1) { n = n < 0 ? -n : n; R = b % n; R.S = (SIGN ? 1 : 0); return R; }
    bits = I32tobin(e);
    R = b;
    for (i = 1; i < (i32)(i32)strlen(bits); i++) { //POWER ALGORITHM => SQUARE AND MULTIPLY
        R = R * R; //SQUARE
        R = ((R < n) ? R : R % n);
        if (bits[i] == '1') { R = R * b; R = (R < n ? R : R % n); } //MULTIPLY
    }
    R.S = (SIGN ? 1 : 0);
    return R;
}
...
/// NUM OUT-LINE /// CHECK IF POINTED NUM OBJECT IS ODD, CODE: NUM a("3.0"); print(is_odd(&a), "\n");  //1
int is_odd(NUM* n) {
    static int FLAG;
    FLAG = -1; //ERROR
    if (is_int(n)) {
...
/// NUM OUT-LINE /// CHECKER FOR PRIME NUMBERS, CODE: NUM a("65537.0"); print(is_prime(&a) ? "is prime!" : "is NOT prime.", "\n"); //is prime!
int is_prime(NUM *n) {
...
char* format(NUM a, int d, char SEP1000, int SIGN) { //DEFAULT ARGs: (int d = 2, char SEP1000 = ',', int SIGN = 0)
    ...
    char* n = exp2num(a);
    I32 DIM = (I32)(sizeof(char) * strlen(n) + (d < 0 ? -d : d) + 2); //NULL, SIGN
    char* s = (char*)malloc(DIM + DIM/3); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION
    char *f = (char*)malloc(DIM + DIM/3); if (freemem() < RAM_SYS) reset();//RAM DYNAMIC ALLOCATION
    char **P = split(n, ".");
    i32 alen_I = (i32)strlen(P[0]);
    i32 alen_F = (i32)strlen(P[1]);
...
char* format0(NUM &a, int d) { 
    char* n = exp2num(a);
    I32 DIM = (I32)(sizeof(char) * strlen(n) + (d < 0 ? -d : d) + 2); //NULL, SIGN
    char* s = (char*)malloc(DIM); if (freemem() < RAM_SYS) reset();  //RAM DYNAMIC ALLOCATION
    char** P = split(n, ".");
    i32 alen_F = (i32)strlen(P[1]);
    strcpy(s, P[0]);
    if (alen_F >= d) {
...
/// NUM OUT-LINE /// ALLOW LEFT OPERAND TO BE int, CODE: NUM a("3.0"); a = 7 + a; a.print("\n"); //10.0
...
/// NUM OUT-LINE /// ALLOW LEFT OPERAND TO BE const *char, CODE: NUM a("3.0"); a = "3.14" + a; a.print("\n"); //6.14
...
/// NUM OUT-LINE /// MODULUS OPERATOR, CODE: NUM a(7); print(String("10.0") % a, "\n"); //3.0
NUM& operator%(String a, NUM& op2) {
    static NUM ZERO;
    NUM A(a);
    if (!A) return ZERO; //(String) ZERO DIVIDEND => ZERO QUOTIENT
...
/// NUM OUT-LINE /// POWER OPERATOR, CODE: NUM e(73); print(3 ^ e, "\n"); //67585198634817523235520443624317923.0
NUM& operator^(int b, NUM& op2) {
    NUM B(b);
    return pwrf_signed(&B, op2.to_i32());
}
/// NUM OUT-LINE /// POWER OPERATOR, CODE: NUM e(73); print("3.0" ^ e, "\n"); //67585198634817523235520443624317923.0
NUM& operator^(const char* b, NUM& op2) {
    NUM B(b);
    return pwrf_signed(&B, op2.to_i32()); 
}
/// NUM OUT-LINE /// POWER OPERATOR, CODE: NUM e(73); print(String("3.0") ^ e, "\n"); //67585198634817523235520443624317923.0
NUM& operator^(String b, NUM& op2) {
    NUM B(b);
    return pwrf_signed(&B, op2.to_i32());
}
/// NUM OUT-LINE /// GREATER THAN OPERATOR, CODE: int a(3); NUM b(4); print(a > b, "\n"); //0 (false)
int operator>(int a, NUM& op2) {
    NUM A(a);
    return gt(&A, &op2);
}
/// NUM OUT-LINE /// GREATER THAN OPERATOR, CODE: const char *a("3.0"); NUM b(3); print(a > b, "\n"); //0 (false)
int operator>(const char* a, NUM& op2) {
    NUM A(a);
    return gt(&A, &op2);
}
/// NUM OUT-LINE /// GREATER THAN OPERATOR, CODE: String a("4.0"); NUM b(3); print(a > b, "\n"); //1 (true)
int operator>(String a, NUM& op2) {
    NUM A(a);
    return gt(&A, &op2);
}
/// NUM OUT-LINE /// GREATER OR EQUAL THAN OPERATOR, CODE: int a(3); NUM b(4); print(a >= b, "\n"); //0 (false)
int operator>=(int a, NUM& op2) {
    NUM A(a);
    return ge(&A, &op2);
}
/// NUM OUT-LINE /// GREATER OR EQUAL THAN OPERATOR, CODE: const char *a("3.0"); NUM b(3); print(a >= b, "\n"); //1 (true)
int operator>=(const char* a, NUM& op2) {
    NUM A(a);
    return ge(&A, &op2);
}
/// NUM OUT-LINE /// GREATER OR EQUAL THAN OPERATOR, CODE: String a("4.0"); NUM b(3); print(a >= b, "\n"); //1 (true)
int operator>=(String a, NUM& op2) {
    NUM A(a);
    return ge(&A, &op2);
}
/// NUM OUT-LINE /// LESS THAN OPERATOR, CODE: int a(3); NUM b(4); print(a < b, "\n"); //1 (true)
int operator<(int a, NUM& op2) {
    NUM A(a);
    return lt(&A, &op2);
}
/// NUM OUT-LINE /// LESS THAN OPERATOR, CODE: const char *a("3.0"); NUM b(3); print(a < b, "\n"); //0 (false)
int operator<(const char* a, NUM& op2) {
    NUM A(a);
    return lt(&A, &op2);
}
/// NUM OUT-LINE /// LESS THAN OPERATOR, CODE: String a("4.0"); NUM b(3); print(a < b, "\n"); //0 (false)
int operator<(String a, NUM& op2) {
    NUM A(a);
    return lt(&A, &op2);
}
/// NUM OUT-LINE /// LESS OR EQUAL THAN OPERATOR, CODE: int a(3); NUM b(4); print(a <= b, "\n"); //1 (true)
int operator<=(int a, NUM& op2) {
    NUM A(a);
    return le(&A, &op2);
}
/// NUM OUT-LINE /// LESS OR EQUAL THAN OPERATOR, CODE: const char *a("3.0"); NUM b(3); print(a <= b, "\n"); //1 (true)
int operator<=(const char* a, NUM& op2) {
    NUM A(a);
    return le(&A, &op2);
}
/// NUM OUT-LINE /// LESS OR EQUAL THAN OPERATOR, CODE: String a("4.0"); NUM b(3); print(a <= b, "\n"); //0 (false)
int operator<=(String a, NUM& op2) {
    NUM A(a);
    return le(&A, &op2);
}
/// NUM OUT-LINE /// EQUAL OPERATOR, CODE: int a(3); NUM b(2); print(a == b, "\n"); //0 (false)
int operator==(int a, NUM& op2) {
    NUM A(a);
    return eq(&A, &op2);
}
...
/// NUM OUT-LINE /// NOT EQUAL OPERATOR, CODE: int a(3); NUM b(2); print(a != b, "\n"); //1 (true)
int operator!=(int a, NUM& op2) {
    NUM A(a);
    return ne(&A, &op2);
}
/// NUM OUT-LINE /// NOT EQUAL OPERATOR, CODE: const char *a("3.0"); NUM b(2); print(a != b, "\n"); //1 (true)
int operator!=(const char* a, NUM& op2) {
    NUM A(a);
    return ne(&A, &op2);
}
...
NUM& operator++(NUM& op1, int) { 
    static NUM old;
    old = op1; //COPY
    op1 = op1 + 1;
    return old;
}
/// NUM OUT-LINE /// UNARY POST-DECREMENT (--), CODE: NUM a(5); print(a--, " "); a.print("\n"); //5.0 4.0
NUM& operator--(NUM& op1, int) { 
    static NUM old;
    old = op1; //COPY
    op1 = op1 - 1;
...
/// NUM OUT-LINE /// NOT OPERATOR, CODE: NUM a(0); print(!a, "\n"); //1
...
NUM& div(NUM &a, NUM &b, int pd) { //DEFAULT ARGs: div(NUM, NUM, int pd = 40); 
    static NUM ZERO;
...
NUM& div(const char* a, const char* b, int pd) { //DEFAULT ARGs: div(const char*, const char*, int pd = 40); 
    static NUM ZERO;
    NUM A(a), B(b);
    if (!A) return ZERO; //(const char*) ZERO DIVIDEND => ZERO QUOTIENT
    if (!B) {
        raise("DIVISION BY ZERO => div", B.sprint_fields());
        //reset(); 
        return ZERO;
...
NUM& div(String a, String b, int pd) { //DEFAULT ARGs: div(String, String, int pd = 40); 
    static NUM ZERO;
    NUM A(a), B(b);
    if (!A) return ZERO; //(const char*) ZERO DIVIDEND => ZERO QUOTIENT
    if (!B) {
...
/// NUM OUT-LINE /// CALCULATOR MODE: DIVISION => CODE: num7::div(1, 3, 6).print("\n"); //0.333333
NUM& div(int a, int b, int pd) { //DEFAULT ARGs: div(int, int, int pd = 40); 
    static NUM ZERO;
    NUM A(a), B(b);
    if (!A) return ZERO; //(const char*) ZERO DIVIDEND => ZERO QUOTIENT
    if (!B) {
...
NUM& sqr(NUM &a, int b) { //DEFAULT b = 9
    return a.sqrt(b);
}
/// NUM OUT-LINE /// CALCULATOR MODE: SQUARE ROOT => CODE: sqr("3.14").print("\n"); //1.772004514
NUM& sqr(const char* a, int b) { //DEFAULT b = 9
    NUM A(a);
    return A.sqrt(b);
}
/// NUM OUT-LINE /// CALCULATOR MODE: SQUARE ROOT => CODE: sqr(String("3.14")).print("\n"); //1.772004514
NUM& sqr(String a, int b) { //DEFAULT b = 9
    NUM A(a);
    return A.sqrt(b);
}
/// NUM OUT-LINE /// CALCULATOR MODE: SQUARE ROOT => CODE: sqr(9).print("\n"); //3.0
NUM& sqr(int a, int b) { //DEFAULT b = 9
    NUM A(a);
    return A.sqrt(b);
}
/// NUM OUT-LINE /// CALCULATOR MODE: ROOT-i => CODE: NUM a("64.36343"); root_i(a, 5).print("\n"); //2.3
NUM& root_i(NUM& a, int b) { //DEFAULT b = 3
    return a.root_i(b);
}
/// NUM OUT-LINE /// CALCULATOR MODE: ROOT-i => CODE: root_i("64.36343", 5).print("\n"); //2.3
NUM& root_i(const char* a, int b) { //DEFAULT b = 3
    NUM A(a);
    return A.root_i(b);
}
/// NUM OUT-LINE /// CALCULATOR MODE: ROOT-i => CODE: root_i(String("64.36343"), 5).print("\n"); //2.3
NUM& root_i(String a, int b) { //DEFAULT b = 3
    NUM A(a);
    return A.root_i(b);
}
/// NUM OUT-LINE /// CALCULATOR MODE: ROOT-i => CODE: root_i(1000).print("\n"); //10.0
NUM& root_i(int a, int b) { //DEFAULT b = 3
    NUM A(a);
    return A.root_i(b);
}
...
/// NUM OUT-LINE /// CALCULATOR MODE: MULTIPLY OR DIVIDE SELF OBJECT FOR 10 POWER int, CODE: NUM a("0.001"); shift(A, 6).print("\n"); //1000.0
NUM& shift(NUM& b, int e) { 
    b.E += e;
    const char* b_E = i32str(b.E);
    b.CE = (char*)realloc(b.CE, (b.len_I + b.len_F + 8 + strlen(b_E)) * sizeof(char)); //'.' + 'e' + '-' + NULL
    if (freemem() < RAM_SYS) reset();
    strcpy(b.CE, b.C); strcat(b.CE, "e"); strcat(b.CE, b_E);
    return b;
}
/// NUM OUT-LINE /// CALCULATOR MODE: RETURN OBJECT MULTIPLIED OR DIVIDED FOR 10 POWER NUM, CODE: NUM a("0.001"), e("6.0"); xe10(a, e).print("\n"); //1000.0
NUM& xe10(NUM& b, NUM& e) {
    static NUM B; B = b;
    return num7::shift(B, e);
}
/// NUM OUT-LINE /// CALCULATOR MODE: RETURN OBJECT MULTIPLIED OR DIVIDED FOR 10 POWER STRING, CODE: NUM a("0.001"); xe10(a, "6.0").print("\n"); //1000.0
NUM& xe10(NUM& b, const char* e) {
    static NUM B; B = b;
    return num7::shift(B, e);
}
/// NUM OUT-LINE /// CALCULATOR MODE: RETURN OBJECT MULTIPLIED OR DIVIDED FOR 10 POWER String, CODE: NUM a("0.001"); xe10(a, String("6.0")).print("\n"); //1000.0
NUM& xe10(NUM& b, String e) {
    static NUM B; B = b;
    return num7::shift(B, e);
}
/// NUM OUT-LINE /// CALCULATOR MODE: RETURN OBJECT MULTIPLIED OR DIVIDED FOR 10 POWER int, CODE: NUM a("0.001"); xe10(a, 6).print("\n"); //1000.0
NUM& xe10(NUM& b, int e) { 
    static NUM B; B = b;
    return num7::shift(B, e);
}
...
/// NUM OUT-LINE /// CALCULATOR MODE: DOUBLED VALUE => CODE: _2x(50).print("\n"); //100.0
NUM& _2x(int a) {
    NUM A(a);
    return A + A;
}
...
NUM& spinon(NUM& rate, NUM all = 1) { //DEFAULT all = 1
    return all * (1 + rate.xe10(-2));
}
/// NUM OUT-LINE /// CALCULATOR MODE: SPIN-ON VALUE => CODE: spinon("22.00", "44.74").round().print("\n"); //54.58
NUM& spinon(const char* RATE, const char* all) { //DEFAULT all = "1.0"
    NUM rate(RATE);
    return all * (1 + rate.shift(-2));
}
/// NUM OUT-LINE /// CALCULATOR MODE: SPIN-ON VALUE => CODE: spinon(String("22.00"), String("44.74")).round().print("\n"); //54.58
NUM& spinon(String RATE, String all) { //DEFAULT all = "1.0"
    NUM rate(RATE);
    return all * (1 + rate.shift(-2));
}
/// NUM OUT-LINE /// CALCULATOR MODE: SPIN-ON VALUE => CODE: spinon(22, 1000).round().print("\n"); //1220
NUM& spinon(int RATE, int all) { //DEFAULT all = 1
    NUM rate(RATE);
    return all * (1 + rate.shift(-2));
}
/// NUM OUT-LINE /// MEMORY FREE => CODE: if (freemem() < RAM_SYS) reset();
I32 freemem() {
  #if defined(__arm__) 
    char top; 
    return &top - sbrk(0);
  ...
  #else
  #warning "ARDUINO BOARD freemem() NOT SUPPORTED BY num7 LIBRARY!"
    return 64*1024; //64KB
  #endif
}
/// NUM OUT-LINE /// SYSTEM RESET => CODE: if (freemem() < RAM_SYS) reset();
void reset() {
  #if defined(__arm__) //ARM systems reset
    NVIC_SystemReset(); 
  #elif defined(ARDUINO_ARCH_ESP32) //ESP32 systems reset
    return ESP.restart();
  #elif defined(__AVR__) //AVR systems reset
    //asm volatile (" jmp 0 ");
    reset_avr();     
  #else
    #warning "ARDUINO BOARD reset() NOT SUPPORTED BY num7 LIBRARY!"
  #endif
}
/// NUM OUT-LINE /// PERFORMANCE RATE ON TIME OR INDEX BASE, CODE: NUM x(1035), y(1035); perf(x, y).print("\n"); //0.0
NUM &perf( NUM v_init, NUM v_end, int time) { //DEFAULT PROTOTYPE: (const char* s, int time = 1)
    static NUM R, T;
    if (time) {
        T = v_init; 
        NUM D(T / v_end);
        if (D >= 1) {
            ...
            T = v_init; return R = (T - v_end) / v_init * 200; }               //10s 6s => 80.0 (% TIME FASTER)
...
/// NUM OUT-LINE /// FACTORIAL COMPUTATION, CODE: fact(1000).print("\n"); //120
NUM& fact(int n) { 
    static NUM N;
    char* P = FACT(n);
    N = P; 
...
/// NUM OUT-LINE /// RETURN FACTORIAL DIGIT NUMBER, CODE: print(fact_digits(1000), " => DIGITS\n"); //2568 => DIGITS
int fact_digits(int n) {
    if (n < 0)  return 0; //FACTORIAL EXISTS ONLY IF n >= 0
    if (n <= 1) return 1; //BASE CASE
    double DIGITS = 0;
    for (int i = 2; i <= n; i++) DIGITS += log10(i); //COMPUTING FACTORIAL DIGIT COUNT
    return (int)floor(DIGITS) + 1;
}
/// NUM OUT-LINE /// NEED: free(res) FACTORIAL COMPUTATION FOR LARGE NUMBER (UP 1e4 DIGITS, RETURN int DIGIT ARRAY), CODE: int N = 100; char* res = FACT(N); print(res); print("\nsize="); print((int)strlen(res), "\n"); free(res); //9332621544394415... (size = 158)
char* FACT(int N) {
    int carry, product, i, j, size, DIM(fact_digits(N));
    char* res = NULL;
    if (N < 0) { raise("IMPOSSIBLE OPERATION => FACT", i32str(N)); return res; }      //-N!
    res = (char*)malloc((DIM + 3) * sizeof(char)); if (freemem() < RAM_SYS) reset(); //RAM DYNAMIC ALLOCATION //NULL, .0
...
int print_fields(NUM &a) {
    String tot = String("S=") + i32str(a.S) + String(" CE=") + a.CE + String(" C=") + a.C;
    tot += String(" E=") + i32str(a.E);
    tot += String(" len_I=") + i32str(a.len_I);
    tot += String(" len_F=") + i32str(a.len_F);
    return Serial.print(tot.c_str());
}
/// NUM OUT-LINE /// PRINT STRING , CODE: NUM a("-123.0e-3"); print_exp(a, "\n"); //-123.0e-3
int print_exp(NUM &a, const char* postfix) { //DEFAULT ARGs: (postfix = "")
    return Serial.print(String(a.S ? "-" : "") + a.CE + String(postfix));
}
/// NUM OUT-LINE /// PRINT NUM STANDARD FORMAT, CODE: NUM a("5.0"); print(a, "\n"); //5.0
int print(NUM &a, const char* postfix, int SIGN) { // DEFAULT ARGs: (postfix = "", SIGN = 0)
    return a.print(postfix, SIGN);
}
/// NUM OUT-LINE /// PRINT STRING , CODE: NUM a("5.0"); print("a = ", a, "\n"); //5.0
int print(const char* prefix, NUM& a, const char* postfix, int SIGN) { //DEFAULT ARGs: (postfix = "", SIGN = 0)
    Serial.print(String(prefix));
    a.print("", SIGN);
    return Serial.print(String(postfix));
}
/// NUM OUT-LINE /// PRINT STRING, CODE: print("A", "B", "C\n"); //ABC
int print(const char* prefix, const char* inter , const char* postfix) { //DEFAULT ARGs: (inter = "", postfix = "")
    return Serial.print(String(prefix) + String(inter) + String(postfix));
}
/// NUM OUT-LINE /// PRINT STRING, CODE: print(String("A"), "B", "C\n"); //ABC
int print(String prefix, const char* inter , const char* postfix) { //DEFAULT ARGs: (inter = "", postfix = "")
    return Serial.print(prefix + String(inter) + String(postfix));
}
/// NUM OUT-LINE /// PRINT int AND POSTFIX STRING , CODE: print(3, "\n"); //3
int print(int a, const char* postfix) { //DEFAULT ARGs: (postfix = "");
    return Serial.print(String(a) + String(postfix));
}
...
NUM& round_bank(NUM &a, i32 d) { //DEFAULT ARGs (NUM&, i32 d = 2)
...
NUM& round_bank(const char* A, i32 d) { //DEFAULT ARGs (NUM&, i32 d = 2)
    NUM a(A);
    return a.round_bank(d);
}
...
NUM& round_floor(NUM& a, i32 decs) { ///DEFAULT ARGs: (NUM&, i32 decs = 0)
    return a.trunk(decs);
}
...
NUM& round_floor(const char* A, i32 decs) { //DEFAULT ARGs: (const char*, i32 decs = 0)
    NUM a(A);
    return a.trunk(decs);
}
...
NUM& round_floor(String A, i32 decs) { ///DEFAULT ARGs: (String, i32 decs = 0)
    NUM a(A);
    return a.trunk(decs);
}
/// NUM OUT-LINE /// ROUND FLOOR OF THE OBJECT TO THE SPECIFIED DIGIT NUMBER (LOSS PRECISION), CODE: print(round_floor(-123, -1), "\n"); //-120.0
NUM& round_floor(int A, i32 decs) { ///DEFAULT ARGs: (int, i32 decs = 0)
    NUM a(A);
    return a.trunk(decs);
}
...
NUM& round_ceil(const char* A, i32 decs) { //DEFAULT ARGs: (const char*, i32 decs = 0)
    NUM a(A);
    return a.round_ceil(decs);
}
...
NUM& dec(NUM& a, int B) { //DEFAULT ARGs: (int B = 1)
    NUM b(B);
    return a -= b;
}
...
/// NUM OUT-LINE /// CALCULATOR MODE: ARRAY SUM, CODE: NUM cart[3] ; cart[0] = "19.32"; cart[1] = "18.37"; cart[2] = "-15.13" ; int elements = 3; sum(cart, elements).print("\n"); //22.56 
...
/// NUM OUT-LINE /// CALCULATOR MODE: ARRAY MEAN, CODE: NUM cart[3] ; cart[0] = "19.32"; cart[1] = "18.37"; cart[2] = "-15.13" ; int elements = 3; mean(cart, elements).round().print("\n"); //7.52
...
/// NUM OUT-LINE /// CALCULATOR MODE: ARRAY MIN, CODE: NUM cart[3] ; cart[0] = "19.32"; cart[1] = "18.37"; cart[2] = "-15.13" ; int elements = 3; min(cart, elements).print("\n"); //-15.13 
...
/// NUM OUT-LINE /// CALCULATOR MODE: ARRAY MAX, CODE: NUM cart[3] ; cart[0] = "19.32"; cart[1] = "18.37"; cart[2] = "-15.13" ; int elements = 3; max(cart, elements).print("\n"); //19.32
...
/// NUM OUT-LINE /// CALCULATOR MODE: ARRAY MIN AND MAX, CODE: NUM cart[3] ; cart[0] = "19.32"; cart[1] = "18.37"; cart[2] = "-15.13" ; int elements = 3; NUM* result = minmax(cart, elements); result[0].print("\n"); result[1].print("\n"); //-15.13 19.32
...
NUM& hex2dec(const char* nx) {
    ...
    for (int i = 0; i < nx_L; i++) { //
        ...
        switch (nx[i]) {
            ...
            case 'C': case 'c': N += 12; break; case 'D': case 'd': N += 13; break;
            case 'E': case 'e': N += 14; break; case 'F': case 'f': N += 15; break;
            default: N += nx[i] - '0'; //DIGIT TO VALUE
        }
    }
    return N;
}
...
NUM& bin2dec(const char* nb) {
    static NUM N; N = 0;
    i32 nb_L = (i32)strlen(nb);
    for (int i = 0; i < nb_L; i++) { N *= 2; N += nb[i] - '0'; } //LEFT SHIFT AND DIGIT TO VALUE
    return N;
}
/// NUM OUT-LINE /// CONVERT DECIMAL NUMBER STRING TO HEXADECIMAL/BINARY STRING -NEED: free(), CODE: char* ram = dec2hex("65536"); print(ram, "\n"); free(ram); //FFFF
char* dec2hex(const char* n, int bin) { //DEFAULT ARGs: (int bin = false)
    return bits(n, bin);
}
...
char* dec2bin(const char* n, int bin) { //DEFAULT ARGs: (int bin = true)
    return bits(n, bin);
}
...
char* hex2bin(const char* nx) {
    i32 nx_L = (i32)strlen(nx);
    char* n = (char*)malloc((nx_L * 4 + 1) * sizeof(char)); //1 => NULL
    if (freemem() < RAM_SYS) reset();
...
/// NUM OUT-LINE /// CONVERT BINARY NUMERIC STRING TO HEXADECIMAL ONE -NEED: free(), CODE: char* ram = bin2hex("1111111111111111"); print(ram, "\n"); free(ram); //FFFF
char* bin2hex(const char* nb) {
    char nib[5];    //nibble => 4-bit + NULL
    nib[4] = '\0'; //NULL => ZERO LENGTH STRING ("")
    i32 nb_L = (i32)strlen(nb);
    char* n = (char*)malloc((nb_L + 4) * sizeof(char)); //1 => NULL AND PADDING ZEROs (MAX. 3)
    if (freemem() < RAM_SYS) reset();
    strcpy(n, nb);
    if (n[0] == '0') { stripi0(n); nb_L = (i32)strlen(n); } //CLEAR ZEROs AND REMEASURE
    int R = nb_L % 4;
    int i = 0, j = 0;
    if (R) {
        switch (R) {
...
NUM& pct(String rate, String all) { //DEFAULT all = "1.0"
    NUM A = all;
    return A.shift(-2) * rate;
}
/// NUM OUT-LINE /// CALCULATOR MODE: PERCENTAGE VALUE => CODE: pct(10, 10000).round().print("\n"); //1000
NUM& pct(int rate, int all) { //DEFAULT all = 1
    NUM A = all;
    return A.shift(-2) * rate;
}  
/// NUM OUT-LINE /// CALCULATOR MODE: WITH THE PERCENTAGE RETURN THE ALL OF SPECIFIED RATE, CODE: NUM rate("2.0"), pct(20); all(rate, pct).print("\n"); //1000.0
NUM& all(NUM& rate, NUM& pct) {
    return pct.xe10(2) / rate;
}
/// NUM OUT-LINE /// CALCULATOR MODE: WITH THE PERCENTAGE RETURN THE ALL OF SPECIFIED RATE, CODE: all("2.0", "20.0").print("\n"); //1000.0
NUM& all(const char* RATE, const char* PCT) {
    NUM rate(RATE), pct(PCT);
    return pct.shift(2) / rate;
}
/// NUM OUT-LINE /// CALCULATOR MODE: WITH THE PERCENTAGE RETURN THE ALL OF SPECIFIED RATE, CODE: all(String("2.0"), String("20.0")).print("\n"); //1000.0
NUM& all(String RATE, String PCT) {
    NUM rate(RATE), pct(PCT);
    return pct.shift(2) / rate;
}
/// NUM OUT-LINE /// CALCULATOR MODE: WITH THE PERCENTAGE RETURN THE ALL OF SPECIFIED RATE, CODE: all(2, 20).print("\n"); //1000.0
NUM& all(int RATE, int PCT) {
    NUM rate(RATE), pct(PCT);
    return pct.shift(2) / rate;
}
/// NUM OUT-LINE /// CALCULATOR MODE: WITH THE ALL, RETURN THE RATE OF SPECIFIED PERCENTAGE, CODE: NUM pct(20), ALL("1_000.0"); rate(pct, ALL).print("\n"); //2.0
NUM& rate(NUM& pct, NUM all = 1) { //DEFAULT all = 1
    return pct.xe10(2) / all;
}
/// NUM OUT-LINE /// CALCULATOR MODE: WITH THE ALL, RETURN THE RATE OF SPECIFIED PERCENTAGE, CODE: rate("20.0", "1000.0").print("\n"); //2.0
NUM& rate(const char* pct, const char* all) { //DEFAULT all = "1.0"
    NUM PCT(pct);
    return PCT.shift(2) / all;
}
/// NUM OUT-LINE /// CALCULATOR MODE: WITH THE ALL, RETURN THE RATE OF SPECIFIED PERCENTAGE, CODE: rate(String("20.0"), String("1000.0")).print("\n"); //2.0
NUM& rate(String pct, String all) { //DEFAULT all = "1.0"
    NUM PCT(pct);
    return PCT.shift(2) / all;
}
/// NUM OUT-LINE /// CALCULATOR MODE: WITH THE ALL, RETURN THE RATE OF SPECIFIED PERCENTAGE, CODE: rate(20, 1000).print("\n"); //2.0
NUM& rate(int pct, int all) { //DEFAULT all = 1
    NUM PCT(pct);
    return PCT.shift(2) / all;
}
/// NUM OUT-LINE /// CALCULATOR MODE: PERTHOUSAND VALUE => CODE: NUM revenue("20_000.00"), rate("2.0"); pth(rate, revenue).print(" => church_fees\n"); //40.0 => church_fees
NUM& pth(NUM& rate, NUM all = 1) { //DEFAULT all = 1
    return all.pth(rate);
}
/// NUM OUT-LINE /// CALCULATOR MODE: PERTHOUSAND VALUE => CODE: pth("2.0", "20_000.00").print(" => church_fees\n"); //40.0 => church_fees
NUM& pth(const char* rate, const char* all) { //DEFAULT all = "1.0"
    NUM ALL(all);
    return ALL.pth(rate);
}
/// NUM OUT-LINE /// CALCULATOR MODE: PERTHOUSAND VALUE => CODE: pth(String("2.0"), String("20_000.00")).print(" => church_fees\n"); //40.0 => church_fees
NUM& pth(String rate, String all) { //DEFAULT all = "1.0"
    NUM ALL(all);
    return ALL.pth(rate);
}
/// NUM OUT-LINE /// CALCULATOR MODE: PERTHOUSAND VALUE => CODE: pth(2, 20000).print(" => church_fees\n"); //40.0 => church_fees
NUM& pth(int rate, int all) { //DEFAULT all = 1
    NUM ALL(all);
    return ALL.pth(rate);
}
/// NUM OUT-LINE /// CALCULATOR MODE: WITH THE ALL, RETURN THE RATE_TH OF SPECIFIED PERTHOUSAND, CODE: NUM pth(20), ALL("10_000.0"); rate_th(pth, ALL).print("\n"); //2.0
NUM& rate_th(NUM& pth, NUM all = 1) { //DEFAULT all = 1
    return pth.xe10(3) / all;
}
/// NUM OUT-LINE /// CALCULATOR MODE: WITH THE ALL, RETURN THE RATE_TH OF SPECIFIED PERTHOUSAND, CODE: rate_th("20.0", "10_000.0").print("\n"); //2.0
NUM& rate_th(const char* pth, const char* all) { //DEFAULT all = "1.0"
    NUM PTH(pth);
    return PTH.shift(3) / all;
}
/// NUM OUT-LINE /// CALCULATOR MODE: WITH THE ALL, RETURN THE RATE_TH OF SPECIFIED PERTHOUSAND, CODE: rate_th(String("20.0"), String("10_000.0")).print("\n"); //2.0
NUM& rate_th(String pth, String all) { //DEFAULT all = "1.0"
    NUM PTH(pth);
    return PTH.shift(3) / all;
}
/// NUM OUT-LINE /// CALCULATOR MODE: WITH THE ALL, RETURN THE RATE_TH OF SPECIFIED PERTHOUSAND, CODE: rate_th(20, 10_000).print("\n"); //2.0
NUM& rate_th(int pth, int all) { //DEFAULT all = 1
    NUM PTH(pth);
    return PTH.shift(3) / all;
}
/// NUM OUT-LINE /// CALCULATOR MODE: WITH THE PERCENTAGE RETURN THE ALL_TH OF SPECIFIED RATE_TH, CODE: NUM pth(20), rate_th("2.0"); all_th(rate_th, pth).print("\n"); //10000.0
NUM& all_th(NUM& rate_th, NUM& 	pth) {
    return pth.xe10(3) / rate_th;
}
/// NUM OUT-LINE /// CALCULATOR MODE: WITH THE PERCENTAGE RETURN THE ALL_TH OF SPECIFIED RATE_TH, CODE: all_th("2.0", "20.0").print("\n"); //10000.0
NUM& all_th(const char* RATE_TH, const char* PTH) {
    NUM rate_th(RATE_TH), pth(PTH);
    return pth.shift(3) / rate_th;
}
/// NUM OUT-LINE /// CALCULATOR MODE: WITH THE PERCENTAGE RETURN THE ALL_TH OF SPECIFIED RATE_TH, CODE: all_th(String("2.0"), String("20.0")).print("\n"); //10000.0
NUM& all_th(String RATE_TH, String PTH) {
    NUM rate_th(RATE_TH), pth(PTH);
    return pth.shift(3) / rate_th;
}
/// NUM OUT-LINE /// CALCULATOR MODE: WITH THE PERCENTAGE RETURN THE ALL_TH OF SPECIFIED RATE_TH, CODE: all_th(2, 20).print("\n"); //10000.0
NUM& all_th(int RATE_TH, int PTH) {
    NUM rate_th(RATE_TH), pth(PTH);
    return pth.shift(3) / rate_th;
}
/// NUM OUT-LINE /// CALCULATOR MODE: MULTIPLY FOR TEN, CODE: NUM a(3); _10x(a).print("\n"); //30.0
NUM& _10x(NUM& a) {
    return a.xe10(1);
}
/// NUM OUT-LINE /// CALCULATOR MODE: MULTIPLY FOR TEN, CODE: NUM a(3); _10x("3.0").print("\n"); //30.0
NUM& _10x(const char *A) {
    NUM a(A);
    return a.xe10(1);
}
/// NUM OUT-LINE /// CALCULATOR MODE: MULTIPLY FOR TEN, CODE: NUM a(3); _10x(String("3.0")).print("\n"); //30.0
NUM& _10x(String A) {
    NUM a(A);
    return a.xe10(1);
}
/// NUM OUT-LINE /// CALCULATOR MODE: MULTIPLY FOR TEN, CODE: NUM a(3); _10x(3).print("\n"); //30.0
NUM& _10x(int A) {
    NUM a(A);
    return a.xe10(1);
}
/// NUM OUT-LINE /// CALCULATOR MODE: MULTIPLY FOR HUNDRED, CODE: NUM a(3); _100x(a).print("\n"); //300.0
NUM& _100x(NUM& a) {
    return a.xe10(2);
}
/// NUM OUT-LINE /// CALCULATOR MODE: MULTIPLY FOR HUNDRED, CODE: NUM a(3); _100x("3.0").print("\n"); //300.0
NUM& _100x(const char* A) {
    NUM a(A);
    return a.xe10(2);
}
/// NUM OUT-LINE /// CALCULATOR MODE: MULTIPLY FOR HUNDRED, CODE: NUM a(3); _100x(String("3.0")).print("\n"); //300.0
NUM& _100x(String A) {
    NUM a(A);
    return a.xe10(2);
}
/// NUM OUT-LINE /// CALCULATOR MODE: MULTIPLY FOR HUNDRED, CODE: NUM a(3); _100x(3).print("\n"); //300.0
NUM& _100x(int A) {
    NUM a(A);
    return a.xe10(2);
}
/// NUM OUT-LINE /// CALCULATOR MODE: MULTIPLY FOR THOUSAND, CODE: NUM a(3); _1000x(a).print("\n"); //3000.0
NUM& _1000x(NUM& a) {
    return a.xe10(3);
}
/// NUM OUT-LINE /// CALCULATOR MODE: MULTIPLY FOR THOUSAND, CODE: NUM a(3); _1000x("3.0").print("\n"); //3000.0
NUM& _1000x(const char* A) {
    NUM a(A);
    return a.xe10(3);
}
/// NUM OUT-LINE /// CALCULATOR MODE: MULTIPLY FOR THOUSAND, CODE: NUM a(3); _1000x(String("3.0")).print("\n"); //3000.0
NUM& _1000x(String A) {
    NUM a(A);
    return a.xe10(3);
}
/// NUM OUT-LINE /// CALCULATOR MODE: MULTIPLY FOR THOUSAND, CODE: NUM a(3); _1000x(3).print("\n"); //3000.0
NUM& _1000x(int A) {
    NUM a(A);
    return a.xe10(3);
}
/// NUM OUT-LINE /// CALCULATOR MODE: DIVIDE FOR TEN, CODE: NUM a(3); _10div(a).print("\n"); //0.3
NUM& _10div(NUM& a) {
    return a.xe10(-1);
}
/// NUM OUT-LINE /// CALCULATOR MODE: DIVIDE FOR TEN, CODE: NUM a(3); _10div("3.0").print("\n"); //0.3
NUM& _10div(const char* A) {
    NUM a(A);
    return a.xe10(-1);
}
/// NUM OUT-LINE /// CALCULATOR MODE: DIVIDE FOR TEN, CODE: NUM a(3); _10div(String("3.0")).print("\n"); //0.3
NUM& _10div(String A) {
    NUM a(A);
    return a.xe10(-1);
}
/// NUM OUT-LINE /// CALCULATOR MODE: DIVIDE FOR TEN, CODE: NUM a(3); _10div(3).print("\n"); //0.3
NUM& _10div(int A) {
    NUM a(A);
    return a.xe10(-1);
}
/// NUM OUT-LINE /// CALCULATOR MODE: DIVIDE FOR HUNDRED, CODE: NUM a(3); _100div(a).print("\n"); //0.03
NUM& _100div(NUM& a) {
    return a.xe10(-2);
}
/// NUM OUT-LINE /// CALCULATOR MODE: DIVIDE FOR HUNDRED, CODE: NUM a(3); _100div("3.0").print("\n"); //0.03
NUM& _100div(const char* A) {
    NUM a(A);
    return a.xe10(-2);
}
/// NUM OUT-LINE /// CALCULATOR MODE: DIVIDE FOR HUNDRED, CODE: NUM a(3); _100div(String("3.0")).print("\n"); //0.03
NUM& _100div(String A) {
    NUM a(A);
    return a.xe10(-2);
}
/// NUM OUT-LINE /// CALCULATOR MODE: DIVIDE FOR HUNDRED, CODE: NUM a(3); _100div(3).print("\n"); //0.03
NUM& _100div(int A) {
    NUM a(A);
    return a.xe10(-2);
}
/// NUM OUT-LINE /// CALCULATOR MODE: DIVIDE FOR THOUSAND, CODE: NUM a(3); _1000div(a).print("\n"); //0.003
NUM& _1000div(NUM& a) {
    return a.xe10(-3);
}
/// NUM OUT-LINE /// CALCULATOR MODE: DIVIDE FOR THOUSAND, CODE: NUM a(3); _1000div("3.0").print("\n"); //0.003
NUM& _1000div(const char* A) {
    NUM a(A);
    return a.xe10(-3);
}
/// NUM OUT-LINE /// CALCULATOR MODE: DIVIDE FOR THOUSAND, CODE: NUM a(3); _1000div(String("3.0")).print("\n"); //0.003
NUM& _1000div(String A) {
    NUM a(A);
    return a.xe10(-3);
}
/// NUM OUT-LINE /// CALCULATOR MODE: DIVIDE FOR THOUSAND, CODE: NUM a(3); _1000div(3).print("\n"); //0.003
NUM& _1000div(int A) {
    NUM a(A);
    return a.xe10(-3);
}
